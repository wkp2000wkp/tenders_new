...
class PHPExcel_Style implements PHPExcel_IComparable
{
        ...
        if (is_array( $pStyles )) {
            if ($this->_isSupervisor) {
                ...
                switch ($selectionType) {
                    case 'COLUMN':
                        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
                            $columnDimension = $this->getActiveSheet()
                                ->getColumnDimensionByColumn( $col );
                    ...
                    case 'ROW':
                        for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row) {
                            $rowDimension = $this->getActiveSheet()
                                ->getRowDimension( $row );
                            $oldXfIndex = $rowDimension->getXfIndex() === null ? 0 : $rowDimension->getXfIndex(); // row without explicit style should be formatted based on default style
    ...
    public function getConditionalStyles () {
        return $this->getActiveSheet()
            ->getConditionalStyles( $this->getActiveCell() );
    }
    ...
    public function setConditionalStyles ($pValue = null) {
        if (is_array( $pValue )) {
            foreach (PHPExcel_Cell::extractAllCellReferencesInRange( $this->getSelectedCells() ) as $cellReference) {
                $this->getActiveSheet()
                    ->setConditionalStyles( $cellReference , $pValue );
            }
        }
        return $this;
    }
    ...
    public function getHashCode () {
        ...
        return md5( $this->getFill()
            ->getHashCode() . $this->getFont()
            ->getHashCode() . $this->getBorders()
            ->getHashCode() . $this->getAlignment()
            ->getHashCode() . $this->getNumberFormat()
            ->getHashCode() . $hashConditionals . $this->getProtection()
