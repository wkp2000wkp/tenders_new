...
class PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter
{
    ...
    public function generateHTMLHeader ($pIncludeStyles = false) {
        ...
        $html .= '    <title>' . htmlspecialchars( $this->_phpExcel
            ->getProperties()
            ->getTitle() ) . '</title>' . "\r\n";
        if ($pIncludeStyles) {
    ...
    public function generateSheetData () {
        ...
        foreach ($sheets as $sheet) {
            ...
            if ($sheet->getPageSetup()
                ...
                $rowsToRepeatAtTop = $sheet->getPageSetup()
            ...
            for ($row = $rowMin; $row <= $rowMax; ++$row) {
                ...
                if (!isset( $this->_isSpannedRow[$sheet->getParent()
                    ->getIndex( $sheet )][$row] )) {
    ...
    public function buildCSS ($generateSurroundingHTML = true) {
        ...
        foreach ($this->_phpExcel
            ->getCellXfCollection() as $index => $style) {
        ...
        foreach ($sheets as $sheet) {
            ...
            if ($rowDimension->getRowHeight() == -1) {
                $pt_height = PHPExcel_Shared_Font::getDefaultRowHeightByFont( $this->_phpExcel
                    ->getDefaultStyle()
                    ->getFont() );
            }
            else {
    ...
    private function _createCSSStyleBorder (PHPExcel_Style_Border $pStyle) {
        ...
        $css = '';
        // Create CSS
        $css .= $this->_mapBorderStyle( $pStyle->getBorderStyle() ) . ' #' . $pStyle->getColor()
            ->getRGB();
        // Return
        return $css;
    }
    ...
    private function _createCSSStyleFill (PHPExcel_Style_Fill $pStyle) {
        ...
        $css = array();
        // Create CSS
        $value = $pStyle->getFillType() == PHPExcel_Style_Fill::FILL_NONE ? 'white' : '#' . $pStyle->getStartColor()
            ->getRGB();
                ...
                if ($cell instanceof PHPExcel_Cell) {
                    ...
                    if ($cell->getValue() instanceof PHPExcel_RichText) {
                        ...
                        if ($this->_preCalculateFormulas) {
                            $cellData = PHPExcel_Style_NumberFormat::toFormattedString( $cell->getCalculatedValue() , $pSheet->getParent()
                            ...
                            $cellData = PHPExcel_Style_NumberFormat::ToFormattedString( $cell->getValue() , $pSheet->getParent()
                                ->getCellXfByIndex( $cell->getXfIndex() )
                                ->getNumberFormat()
                                ->getFormatCode() , array(
                    ...
                    if (!$this->_useInlineCss) {
                        ...
                        $sharedStyle = $pSheet->getParent()
                            ->getCellXfByIndex( $cell->getXfIndex() );
                        if ($sharedStyle->getAlignment()
                            ->getHorizontal() == PHPExcel_Style_Alignment::HORIZONTAL_GENERAL && isset( $this->_cssStyles['.' . $cell->getDataType()]['text-align'] )) {
                            $cssClass['text-align'] = $this->_cssStyles['.' . $cell->getDataType()]['text-align'];
                        }
                    }
                }
                // Hyperlink?
                if ($pSheet->hyperlinkExists( $coordinate ) && !$pSheet->getHyperlink( $coordinate )
                    ->isInternal()) {
                    $cellData = '<a href="' . htmlspecialchars( $pSheet->getHyperlink( $coordinate )
                        ->getUrl() ) . '" title="' . htmlspecialchars( $pSheet->getHyperlink( $coordinate )
                        ->getTooltip() ) . '">' . $cellData . '</a>';
                }
                // Should the cell be written or is it swallowed by a rowspan or colspan?
                $writeCell = !(isset( $this->_isSpannedCell[$pSheet->getParent()
                    ->getIndex( $pSheet )][$pRow + 1][$colNum] ) && $this->_isSpannedCell[$pSheet->getParent()
                    ->getIndex( $pSheet )][$pRow + 1][$colNum]);
                // Colspan and Rowspan
                $colspan = 1;
                $rowspan = 1;
                if (isset( $this->_isBaseCell[$pSheet->getParent()
                    ->getIndex( $pSheet )][$pRow + 1][$colNum] )) {
                    $spans = $this->_isBaseCell[$pSheet->getParent()
                        ->getIndex( $pSheet )][$pRow + 1][$colNum];
