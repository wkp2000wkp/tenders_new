...
class PHPExcel_Writer_Serialized implements PHPExcel_Writer_IWriter
{
    ...
    public function save ($pFilename = null) {
        if (!is_null( $this->_spreadSheet )) {
            ...
            // Add media
            $sheetCount = $this->_spreadSheet
                ->getSheetCount();
            for ($i = 0; $i < $sheetCount; ++$i) {
                for ($j = 0; $j < $this->_spreadSheet
                    ->getSheet( $i )
                    ...
                    if ($this->_spreadSheet
                        ->getSheet( $i )
                        ->getDrawingCollection()
                        ->offsetGet( $j ) instanceof PHPExcel_Worksheet_BaseDrawing) {
                        $imgTemp = $this->_spreadSheet
                            ->getSheet( $i )
                            ->getDrawingCollection()
                            ->offsetGet( $j );
    ...
    private function _writeSerialized (PHPExcel $pPHPExcel = null, $pFilename = '') {
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            for ($j = 0; $j < $pPHPExcel->getSheet( $i )
                ->getDrawingCollection()
                ->count(); ++$j) {
                if ($pPHPExcel->getSheet( $i )
                    ->getDrawingCollection()
                    ->offsetGet( $j ) instanceof PHPExcel_Worksheet_BaseDrawing) {
                    $imgTemp = & $pPHPExcel->getSheet( $i )
                        ->getDrawingCollection()
                        ->offsetGet( $j );
