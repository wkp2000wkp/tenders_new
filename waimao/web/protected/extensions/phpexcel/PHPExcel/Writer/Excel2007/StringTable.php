    ...
    public function createStringTable ($pSheet = null, $pExistingTable = null) {
        if (!is_null( $pSheet )) {
            ...
            foreach ($pSheet->getCellCollection() as $cellID) {
                ...
                else if ($cell->getValue() instanceof PHPExcel_RichText && !isset( $aFlippedStringTable[$cell->getValue()
                    ->getHashCode()] ) && !is_null( $cell->getValue() )) {
                    $aStringTable[] = $cell->getValue();
                    $aFlippedStringTable[$cell->getValue()
                        ->getHashCode()] = 1;
                }
            }
            // Return
    ...
    public function writeRichText (PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_RichText $pRichText = null) {
        ...
        foreach ($elements as $element) {
            ...
            if ($element instanceof PHPExcel_RichText_Run) {
                ...
                if ($element->getFont()
                    ->getSuperScript() || $element->getFont()
                    ->getSubScript()) {
