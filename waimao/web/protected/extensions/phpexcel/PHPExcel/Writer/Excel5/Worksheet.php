...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    public function __construct ($BIFF_version, &$str_total, &$str_unique, &$str_table, &$colors, $parser, $preCalculateFormulas, $phpSheet) {
        ...
        $this->_countCellStyleXfs = count( $phpSheet->getParent()
    ...
    function close () {
        ...
        // Column dimensions
        $columnDimensions = $this->_phpSheet
            ->getColumnDimensions();
        for ($i = 0; $i < 256; ++$i) {
            $hidden = 0;
            $level = 0;
            ...
            if ($this->_phpSheet
                ...
                ->getWidth() >= 0) {
                $width = $this->_phpSheet
                    ->getDefaultColumnDimension()
                    ->getWidth();
            }
            else {
                $width = PHPExcel_Shared_Font::getDefaultColumnWidthByFont( $this->_phpSheet
                    ->getParent()
                    ->getDefaultStyle()
                    ->getFont() );
            }
        ...
        if ($this->_BIFF_version == 0x0500) {
            for ($i = 0; $i < $num_sheets; ++$i) {
                $this->_writeExternsheet( $this->_phpSheet
                    ->getParent()
                    ->getSheet( $i )
                    ->getTitle() );
            }
        ...
        foreach ($this->_phpSheet
            ...
            if ($cell->getValue() instanceof PHPExcel_RichText) {
                $this->_writeString( $row , $column , $cell->getValue()
                    ->getPlainText() , $xfIndex );
            }
            else {
    ...
    private function _writeWindow2 () {
        ...
        $fDspGrid = $this->_phpSheet
            ->getShowGridlines() ? 1 : 0; // 1
        $fDspRwCol = $this->_phpSheet
            ->getShowRowColHeaders() ? 1 : 0; // 2
        $fFrozen = $this->_phpSheet
            ->getFreezePane() ? 1 : 0; // 3
        $fDspZeros = 1; // 4
        $fDefaultHdr = 1; // 5
        $fArabic = $this->_phpSheet
            ->getRightToLeft() ? 1 : 0; // 6
    ...
    private function _writeDefaultRowHeight () {
        $defaultRowHeight = $this->_phpSheet
    ...
    private function _writeSelection () {
        ...
        $selectedCells = $this->_phpSheet
            ->getSelectedCells();
        $selectedCells = PHPExcel_Cell::splitRange( $this->_phpSheet
            ->getSelectedCells() );
...
0x0000 ); // not used
    ...
    private function _writeSetup () {
        ...
        $iPaperSize = $this->_phpSheet
            ->getPageSetup()
            ->getPaperSize(); // Paper size
        $iScale = $this->_phpSheet
            ->getPageSetup()
            ->getScale() ? $this->_phpSheet
            ->getPageSetup()
            ->getScale() : 100; // Print scaling factor
        $iPageStart = 0x01; // Starting page number
        $iFitWidth = (int) $this->_phpSheet
            ->getPageSetup()
            ->getFitToWidth(); // Fit to number of pages wide
        $iFitHeight = (int) $this->_phpSheet
            ->getPageSetup()
            ->getFitToHeight(); // Fit to number of pages high
        $grbit = 0x00; // Option flags
        $iRes = 0x0258; // Print resolution
        $iVRes = 0x0258; // Vertical print resolution
        $numHdr = $this->_phpSheet
            ->getPageMargins()
            ->getHeader(); // Header Margin
        $numFtr = $this->_phpSheet
            ->getPageMargins()
            ->getFooter(); // Footer Margin
    ...
    private function _writeHeader () {
		...
		*/
        if ($this->_BIFF_version == 0x0600) {
            $recordData = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong( $this->_phpSheet
                ->getHeaderFooter()
                ->getOddHeader() );
            $length = strlen( $recordData );
        }
        else {
            $cch = strlen( $this->_phpSheet
                ->getHeaderFooter()
                ->getOddHeader() ); // Length of header string
            $length = 1 + $cch; // Bytes to follow
            $data = pack( "C" , $cch );
            $recordData = $data . $this->_phpSheet
                ->getHeaderFooter()
                ->getOddHeader();
        }
        $header = pack( "vv" , $record , $length );
    ...
    private function _writeFooter () {
		...
		*/
        if ($this->_BIFF_version == 0x0600) {
            ...
            $cch = strlen( $this->_phpSheet
                ->getHeaderFooter()
                ->getOddFooter() ); // Length of footer string
            $length = 1 + $cch;
            $data = pack( "C" , $cch );
            $recordData = $data . $this->_phpSheet
                ->getHeaderFooter()
    ...
    private function _writeHcenter () {
        ...
        $fHCenter = $this->_phpSheet
            ->getPageSetup()
            ->getHorizontalCentered() ? 1 : 0; // Horizontal centering
        $header = pack( "vv" , $record , $length );
    ...
    private function _writeVcenter () {
        ...
        $length = 0x0002; // Bytes to follow
        $fVCenter = $this->_phpSheet
            ->getPageSetup()
            ->getVerticalCentered() ? 1 : 0; // Horizontal centering
        $header = pack( "vv" , $record , $length );
    ...
    private function _writeMarginLeft () {
        ...
        $margin = $this->_phpSheet
            ->getPageMargins()
            ->getLeft(); // Margin in inches
        $header = pack( "vv" , $record , $length );
        $data = pack( "d" , $margin );
        if (PHPExcel_Writer_Excel5_BIFFwriter::getByteOrder()) { // if it's Big Endian
            $data = strrev( $data );
    ...
    private function _writePrintGridlines () {
        ...
        $length = 0x0002; // Bytes to follow
        $fPrintGrid = $this->_phpSheet
            ->getPrintGridlines() ? 1 : 0; // Boolean flag
        $header = pack( "vv" , $record , $length );
    ...
    private function _writeGridset () {
        ...
        $length = 0x0002; // Bytes to follow
        $fGridSet = !$this->_phpSheet
            ->getPrintGridlines(); // Boolean flag
        $header = pack( "vv" , $record , $length );
...
0x0000 ); // length of ftEnd data
    ...
    private function _writeDataValidity () {
        ...
        $dataValidationCollection = $this->_phpSheet
