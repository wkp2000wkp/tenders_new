    ...
    public function close () {
        ...
        switch (get_class( $this->_object )) {
            ...
            case 'PHPExcel_Shared_Escher_DggContainer':
                ...
                // dgg data
                $dggData = pack( 'VVVV' , $this->_object
                    ->getSpIdMax() , // maximum shape identifier increased by one
$this->_object
                    ->getCDgSaved() + 1 , // number of file identifier clusters increased by one
$this->_object
                    ->getCSpSaved() , $this->_object
                    ->getCDgSaved() ); // count total number of drawings saved
                ...
                if ($bstoreContainer = $this->_object
                    ->getBstoreContainer()) {
            ...
            case 'PHPExcel_Shared_Escher_DggContainer_BstoreContainer':
                ...
                // write the record
                $recVer = 0xF;
                $recInstance = count( $this->_object
                    ->getBSECollection() );
                $recType = 0xF001;
                $length = strlen( $innerData );
                $recVerInstance = $recVer;
            ...
            case 'PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE':
                ...
                // initialize
                $data = '';
                $btWin32 = $this->_object
                    ->getBlipType();
                $btMacOS = $this->_object
                    ->getBlipType();
            ...
            case 'PHPExcel_Shared_Escher_DgContainer':
                ...
                $countShapes = count( $this->_object
                    ->getSpgrContainer()
                    ->getChildren() );
                $innerData .= $header . pack( 'VV' , $countShapes , $this->_object
                    ->getLastSpId() );
                //$innerData .= $header . pack('VV', 0, 0);
                // write the spgrContainer
                if ($spgrContainer = $this->_object
                    ->getSpgrContainer()) {
            ...
            case 'PHPExcel_Shared_Escher_DgContainer_SpgrContainer':
                ...
                $spOffsets = array();
                // treat the inner data
                foreach ($this->_object
                    ->getChildren() as $spContainer) {
            ...
            case 'PHPExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer':
                ...
                $recVer = 0x2;
                $recInstance = $this->_object
                    ->getSpType(); // shape type
                $recType = 0xF00A;
                $length = 0x00000008;
                $recVerInstance = $recVer;
                $recVerInstance |= $recInstance << 4;
                $header = pack( 'vvV' , $recVerInstance , $recType , $length );
                $data .= $header . pack( 'VV' , $this->_object
                    ->getSpId() , $this->_object
                    ->getSpgr() ? 0x0005 : 0x0A00 );
                // the options
                if ($this->_object
                    ->getOPTCollection()) {
                    $optData = '';
                    ...
                    $recType = 0xF00B;
                    foreach ($this->_object
                        ->getOPTCollection() as $property => $value) {
                ...
                if ($this->_object
                    ...
                    $recType = 0xF010;
                    // start coordinates
                    list($column, $row) = PHPExcel_Cell::coordinateFromString( $this->_object
                        ->getStartCoordinates() );
                    $c1 = PHPExcel_Cell::columnIndexFromString( $column ) - 1;
                    $r1 = $row - 1;
                    // start offsetX
                    $startOffsetX = $this->_object
                        ->getStartOffsetX();
                    // start offsetY
                    $startOffsetY = $this->_object
                        ->getStartOffsetY();
                    // end coordinates
                    list($column, $row) = PHPExcel_Cell::coordinateFromString( $this->_object
                        ->getEndCoordinates() );
                    ...
                    $r2 = $row - 1;
                    // end offsetX
                    $endOffsetX = $this->_object
                        ->getEndOffsetX();
                    // end offsetY
                    $endOffsetY = $this->_object
                        ->getEndOffsetY();
                    $clientAnchorData = pack( 'vvvvvvvvv' , 0x02 , $c1 , $startOffsetX , $r1 , $startOffsetY , $c2 , $endOffsetX , $r2 , $endOffsetY );
