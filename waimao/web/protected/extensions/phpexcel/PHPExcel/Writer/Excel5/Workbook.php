...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    public function __construct (PHPExcel $phpExcel = null, $BIFF_version = 0x0600, &$str_total, &$str_unique, &$str_table, &$colors, $parser) {
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            ...
            $this->_parser
                ->setExtSheet( $phpSheet->getTitle() , $i ); // Register worksheet name with parser
            // for BIFF8
            ...
            if ($phpSheet->isTabColorSet()) {
                $this->_addColor( $phpSheet->getTabColor()
                    ->getRGB() );
            }
        }
    }
    ...
    public function addXfWriter ($style, $isStyleXf = false) {
        ...
        if (isset( $this->_addedFonts[$fontHashCode] )) {
            ...
            $fontWriter->setColorIndex( $this->_addColor( $style->getFont()
                ->getColor()
                ->getRGB() ) );
        ...
        $xfWriter->setFgColor( $this->_addColor( $style->getFill()
            ->getStartColor()
            ->getRGB() ) );
        $xfWriter->setBgColor( $this->_addColor( $style->getFill()
            ->getEndColor()
            ->getRGB() ) );
        $xfWriter->setBottomColor( $this->_addColor( $style->getBorders()
            ->getBottom()
            ->getColor()
            ->getRGB() ) );
        $xfWriter->setTopColor( $this->_addColor( $style->getBorders()
            ->getTop()
            ->getColor()
            ->getRGB() ) );
        $xfWriter->setRightColor( $this->_addColor( $style->getBorders()
            ->getRight()
            ->getColor()
            ->getRGB() ) );
        $xfWriter->setLeftColor( $this->_addColor( $style->getBorders()
            ->getLeft()
            ->getColor()
            ->getRGB() ) );
        $xfWriter->setDiagColor( $this->_addColor( $style->getBorders()
            ->getDiagonal()
            ->getColor()
            ->getRGB() ) );
        ...
        if ($style->getNumberFormat()
            ...
            $numberFormatHashCode = $style->getNumberFormat()
                ->getHashCode();
            ...
            $numberFormatIndex = (int) $style->getNumberFormat()
    ...
    public function writeWorkbook ($pWorksheetSizes = null) {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            $this->_writeBoundsheet( $this->_phpExcel
    ...
    private function _writeExterns () {
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            $this->_writeExternsheet( $phpExcel->getSheet( $i )
                ->getTitle() );
        }
    }
...
$rowmin , $rowmax , $colmin , $colmax );
        ...
        if (count( $this->_phpExcel
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                for ($i = 0; $i < count( $range ); $i++) {
                    $range[$i][0] = '\'' . str_replace( "'" , "''" , $namedRange->getWorksheet()
                ...
                try {
                    ...
                    if ($namedRange->getLocalOnly()) {
                        ...
                        $scope = $this->_phpExcel
                            ->getIndex( $namedRange->getScope() ) + 1;
                    }
                    else {
                        // global scope
                        $scope = 0;
    ...
    private function _writeWindow1 () {
        ...
        $itabCur = $this->_phpExcel
            ->getActiveSheetIndex(); // Active worksheet
        $header = pack( "vv" , $record , $length );
    ...
    private function _writeSupbookInternal () {
        ...
        $header = pack( "vv" , $record , $length );
        $data = pack( "vv" , $this->_phpExcel
            ->getSheetCount() , 0x0401 );
