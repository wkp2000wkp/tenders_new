...
class PHPExcel_Writer_PDF extends PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter
{
     ...
     * @var array
     */
    private static $_paperSizes = array( //	Excel Paper Size													TCPDF Paper Size
    ...
    public function save ($pFilename = null) {
        ...
        $pdf->writeHTML( $html );
        // Document info
        $pdf->SetTitle( $this->_phpExcel
            ->getProperties()
            ->getTitle() );
        $pdf->SetAuthor( $this->_phpExcel
            ->getProperties()
            ->getCreator() );
        $pdf->SetSubject( $this->_phpExcel
            ->getProperties()
            ->getSubject() );
        $pdf->SetKeywords( $this->_phpExcel
            ->getProperties()
            ->getKeywords() );
        $pdf->SetCreator( $this->_phpExcel
            ->getProperties()
            ->getCreator() );
        // Write to file
