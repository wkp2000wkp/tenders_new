...
class CGoogleApi {
    ...
    public static function init ($apiKey = null) {
        if ($apiKey === null)
            return CHtml::scriptFile( 
                    self::BOOTSTRAP_URL );
        else
            return CHtml::scriptFile( 
                    self::BOOTSTRAP_URL . '?key=' . $apiKey );
    }
    ...
    public static function load ($name, $version = '1', $options = array()) {
        if (empty( $options ))
            ...
            return "google.load(\"{$name}\",\"{$version}\"," . CJavaScript::encode( 
                    $options ) . ");";
    }
    ...
    public static function register ($name, $version = '1', $options = array(), $apiKey = null) {
        ...
        $js = self::load( $name , $version , 
                $options );
        $cs->registerScript( $name , $js , 
