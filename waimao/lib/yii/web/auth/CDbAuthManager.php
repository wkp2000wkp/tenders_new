...
class CDbAuthManager extends CAuthManager {
    ...
    protected function checkAccessRecursive ($itemName, $userId, $params, $assignments) {
        if (($item = $this->getAuthItem( 
                $itemName )) === null) return false;
        Yii::trace( 
                'Checking permission "' . $item->getName() . '"' , 
                'system.web.auth.CDbAuthManager' );
        if ($this->executeBizRule( 
                $item->getBizRule() , 
                $params , 
                $item->getData() )) {
            if (in_array( 
                    $itemName , 
                    $this->defaultRoles )) return true;
            if (isset( 
                    $assignments[$itemName] )) {
                $assignment = $assignments[$itemName];
                if ($this->executeBizRule( 
                        $assignment->getBizRule() , 
                        $params , 
                        $assignment->getData() )) return true;
            }
    ...
    public function addItemChild ($itemName, $childName) {
        if ($itemName === $childName) throw new CException( 
                Yii::t( 
                        'yii' , 
        ...
        $command = $this->db
            ->createCommand( 
                $sql );
        $command->bindValue( ':name1' , 
                $itemName );
        $command->bindValue( ':name2' , 
                $childName );
        ...
        if (count( $rows ) == 2) {
            ...
            $this->checkItemChildType( 
                    $parentType , 
                    $childType );
            if ($this->detectLoop( 
                    $itemName , 
                    $childName )) throw new CException( 
                    Yii::t( 
                            'yii' , 
    ...
    public function removeItemChild ($itemName, $childName) {
        ...
        $command = $this->db
            ->createCommand( 
                $sql );
        $command->bindValue( ':parent' , 
                $itemName );
        $command->bindValue( ':child' , 
                $childName );
    ...
    public function getItemChildren ($names) {
        ...
        foreach ($this->db
            ...
            ->queryAll() as $row) {
            if (($data = @unserialize( 
                    $row['data'] )) === false) $data = null;
            $children[$row['name']] = new CAuthItem( 
                    $this , 
                    $row['name'] , 
                    $row['type'] , 
                    $row['description'] , 
    ...
    public function assign ($itemName, $userId, $bizRule = null, $data = null) {
        if ($this->usingSqlite() && $this->getAuthItem( 
                $itemName ) === null) throw new CException( 
                Yii::t( 
                        'yii' , 
        ...
        $command = $this->db
            ->createCommand( 
                $sql );
        $command->bindValue( ':itemname' , 
                $itemName );
        $command->bindValue( ':userid' , $userId );
        $command->bindValue( ':bizrule' , 
                $bizRule );
        $command->bindValue( ':data' , 
                serialize( 
                        $data ) );
        $command->execute();
        return new CAuthAssignment( $this , 
                $itemName , 
                $userId , 
                $bizRule , 
                $data );
    ...
    public function createAuthItem ($name, $type, $description = '', $bizRule = null, $data = null) {
        ...
        $command->bindValue( ':data' , 
                serialize( 
                        $data ) );
        $command->execute();
        return new CAuthItem( $this , $name , 
                $type , 
                $description , 
                $bizRule , 
                $data );
    ...
    public function saveAuthItem ($item, $oldName = null) {
        ...
        $command = $this->db
            ->createCommand( 
                $sql );
        $command->bindValue( ':type' , 
                $item->getType() );
        $command->bindValue( ':name' , 
                $oldName === null ? $item->getName() : $oldName );
        $command->bindValue( ':newName' , 
                $item->getName() );
        $command->bindValue( ':description' , 
                $item->getDescription() );
        $command->bindValue( ':bizrule' , 
    ...
    public function clearAll () {
        ...
        $this->db
            ->createCommand( 
                "DELETE FROM {$this->itemChildTable}" )
            ->execute();
        $this->db
            ->createCommand( 
    ...
    public function clearAuthAssignments () {
        $this->db
            ->createCommand( 
                "DELETE FROM {$this->assignmentTable}" )
            ->execute();
    }
    ...
    protected function detectLoop ($itemName, $childName) {
        ...
        foreach ($this->getItemChildren( 
                $childName ) as $child) {
            if ($this->detectLoop( 
                    $itemName , 
                    $child->getName() )) return true;
        }
        return false;
    }
    ...
    protected function getDbConnection () {
        if ($this->db !== null)
            return $this->db;
        else if (($this->db = Yii::app()->getComponent( 
                $this->connectionID )) instanceof CDbConnection)
            return $this->db;
        else
            throw new CException( 
                    Yii::t( 
