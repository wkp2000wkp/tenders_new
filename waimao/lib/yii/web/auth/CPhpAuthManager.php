...
class CPhpAuthManager extends CAuthManager {
    ...
    public function init () {
        parent::init();
        if ($this->authFile === null) $this->authFile = Yii::getPathOfAlias( 
                'application.data.auth' ) . '.php';
        $this->load();
    }
    ...
    public function checkAccess ($itemName, $userId, $params = array()) {
        if (!isset( $this->_items[$itemName] )) return false;
        $item = $this->_items[$itemName];
        Yii::trace( 
                'Checking permission "' . $item->getName() . '"' , 
                'system.web.auth.CPhpAuthManager' );
        if ($this->executeBizRule( 
            ...
            foreach ($this->_children as $parentName => $children) {
                if (isset( 
                        $children[$itemName] ) && $this->checkAccess( 
                        $parentName , 
                        $userId , 
                        $params )) return true;
            }
    ...
    public function addItemChild ($itemName, $childName) {
        if (!isset( $this->_items[$childName] , 
                $this->_items[$itemName] )) throw new CException( 
                Yii::t( 
                        'yii' , 
        ...
        if (isset( 
                $this->_children[$itemName][$childName] )) throw new CException( 
                Yii::t( 
                        'yii' , 
    ...
    public function getItemChildren ($names) {
        if (is_string( $names )) return isset( 
                $this->_children[$names] ) ? $this->_children[$names] : array();
        $children = array();
        foreach ($names as $name) {
            if (isset( 
                    $this->_children[$name] )) $children = array_merge( 
                    $children , 
                    $this->_children[$name] );
        }
        return $children;
    ...
    public function getAuthItems ($type = null, $userId = null) {
        if ($type === null && $userId === null) return $this->_items;
        $items = array();
        if ($userId === null) {
            foreach ($this->_items as $name => $item) {
                if ($item->getType() == $type) $items[$name] = $item;
            }
        }
        else if (isset( 
            ...
            foreach ($this->_assignments[$userId] as $assignment) {
                ...
                if (isset( 
                        $this->_items[$name] ) && ($type === null || $this->_items[$name]
                    ->getType() == $type)) $items[$name] = $this->_items[$name];
            }
        }
        return $items;
    }
    ...
    public function createAuthItem ($name, $type, $description = '', $bizRule = null, $data = null) {
        if (isset( $this->_items[$name] )) throw new CException( 
                Yii::t( 
                        'yii' , 
                        'Unable to add an item whose name is the same as an existing item.' ) );
        return $this->_items[$name] = new CAuthItem( 
                $this , 
                $name , 
                $type , 
                $description , 
...
{
            if (isset( 
                    $this->_items[$newName] )) throw new CException( 
                    Yii::t( 
                            'yii' , 
    ...
    public function load () {
        $this->clearAll();
        $items = $this->loadFromFile( 
                $this->authFile );
        foreach ($items as $name => $item)
            $this->_items[$name] = new CAuthItem( 
                    $this , 
                    $name , 
                    $item['type'] , 
                    $item['description'] , 
        ...
        foreach ($items as $name => $item) {
            if (isset( 
                ...
                foreach ($item['children'] as $childName) {
                    if (isset( 
                            $this->_items[$childName] )) $this->_children[$name][$childName] = $this->_items[$childName];
                }
            }
            if (isset( 
                    $item['assignments'] )) {
    ...
    protected function detectLoop ($itemName, $childName) {
        ...
        if (!isset( $this->_children[$childName] , 
                $this->_items[$itemName] )) return false;
