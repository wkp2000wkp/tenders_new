...
class CWebUser extends CApplicationComponent implements IWebUser {
    ...
    public function init () {
        ...
        else if ($this->autoRenewCookie && $this->allowAutoLogin) $this->renewCookie();
        $this->updateFlash();
    }
        ...
        if ($this->beforeLogout()) {
            if ($this->allowAutoLogin) {
                Yii::app()->getRequest()
                    ->getCookies()
                    ->remove( 
                        $this->getStateKeyPrefix() );
                if ($this->identityCookie !== null) {
                    $cookie = $this->createIdentityCookie( 
                            $this->getStateKeyPrefix() );
                    $cookie->value = null;
                    $cookie->expire = 0;
                    Yii::app()->getRequest()
    ...
    public function getReturnUrl () {
        return $this->getState( '__returnUrl' , 
                Yii::app()->getRequest()
                    ->getScriptUrl() );
    }
    ...
    public function loginRequired () {
        ...
        if (!$request->getIsAjaxRequest()) $this->setReturnUrl( 
                $request->getUrl() );
        if (($url = $this->loginUrl) !== null) {
            if (is_array( $url )) {
                $route = isset( 
                        $url[0] ) ? $url[0] : $app->defaultController;
                $url = $app->createUrl( 
                        $route , 
                        array_splice( 
                                $url , 
    ...
    protected function renewCookie () {
        $cookies = Yii::app()->getRequest()
            ->getCookies();
        $cookie = $cookies->itemAt( 
                $this->getStateKeyPrefix() );
        if ($cookie && !empty( $cookie->value ) && ($data = Yii::app()->getSecurityManager()
            ->validateData( 
                $cookie->value )) !== false) {
            $data = @unserialize( 
                    $data );
    ...
    protected function saveToCookie ($duration) {
        ...
        $cookie->value = $app->getSecurityManager()
            ->hashData( 
                serialize( 
                        $data ) );
        $app->getRequest()
    ...
    public function getFlash ($key, $defaultValue = null, $delete = true) {
        $value = $this->getState( 
                self::FLASH_KEY_PREFIX . $key , 
                $defaultValue );
        if ($delete) $this->setFlash( $key , 
                null );
        return $value;
    }
    ...
    public function setFlash ($key, $value, $defaultValue = null) {
        ...
        if ($value === $defaultValue)
            ...
            $counters[$key] = 0;
        $this->setState( self::FLASH_COUNTERS , 
                $counters , 
                array() );
    }
    ...
    protected function saveIdentityStates () {
        $states = array();
        foreach ($this->getState( 
                self::STATES_VAR , 
                array() ) as $name => $dummy)
            $states[$name] = $this->getState( 
                    $name );
        return $states;
    }
    ...
    public function checkAccess ($operation, $params = array(), $allowCaching = true) {
        if ($allowCaching && $params === array() && isset( 
                $this->_access[$operation] ))
            return $this->_access[$operation];
        else
            return $this->_access[$operation] = Yii::app()->getAuthManager()
                ->checkAccess( 
                    $operation , 
                    $this->getId() , 
                    $params );
