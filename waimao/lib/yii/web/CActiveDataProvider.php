...
class CActiveDataProvider extends CDataProvider {
    ...
    public function getCriteria () {
        if ($this->_criteria === null) $this->_criteria = new CDbCriteria();
    ...
    public function setCriteria ($value) {
        $this->_criteria = $value instanceof CDbCriteria ? $value : new CDbCriteria( 
                $value );
    }
    ...
    public function getSort () {
        if (($sort = parent::getSort()) !== false) $sort->modelClass = $this->modelClass;
        return $sort;
    }
    ...
    protected function fetchData () {
        ...
        if (($pagination = $this->getPagination()) !== false) {
            if ($baseCriteria !== null) $this->model
                ->setDbCriteria( 
                    clone $baseCriteria );
            $pagination->setItemCount( 
                    $this->getTotalItemCount() );
            $pagination->applyLimit( 
                    $criteria );
        }
        if (($sort = $this->getSort()) !== false) $sort->applyOrder( 
                $criteria );
        $this->model
            ->setDbCriteria( 
                $baseCriteria );
