 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CTreeView.php 1839 2010-02-26 04:08:14Z qiang.xue $
...
class CTreeView extends CWidget {
    ...
    public function init () {
        ...
        if ($this->url !== null) $this->url = CHtml::normalizeUrl( 
                $this->url );
        $cs = Yii::app()->getClientScript();
        $cs->registerCoreScript( 'treeview' );
        $options = $this->getClientOptions();
        $options = $options === array() ? '{}' : CJavaScript::encode( 
                $options );
        $cs->registerScript( 
                'Yii.CTreeView#' . $id , 
                "jQuery(\"#{$id}\").treeview($options);" );
        if ($this->cssFile === null)
            $cs->registerCssFile( 
                    $cs->getCoreScriptUrl() . '/treeview/jquery.treeview.css' );
        else if ($this->cssFile !== false) $cs->registerCssFile( 
                $this->cssFile );
        echo CHtml::tag( 'ul' , 
                $this->htmlOptions , 
                false , 
    ...
    protected function getClientOptions () {
        ...
        foreach (array(
            ...
            if ($this->$name !== null) $options[$name] = $this->$name;
        }
        return $options;
    }
