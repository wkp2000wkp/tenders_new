...
class CAutoComplete extends CInputWidget {
    ...
    public function registerClientScript () {
        ...
        $options = $acOptions === array() ? '{}' : CJavaScript::encode( 
                $acOptions );
        ...
        $cs->registerScript( 
                'Yii.CAutoComplete#' . $id , 
                "jQuery(\"#{$id}\").legacyautocomplete($data,{$options}){$this->methodChain};" );
    ...
    public static function registerCssFile ($url = null) {
        $cs = Yii::app()->getClientScript();
        if ($url === null) $url = $cs->getCoreScriptUrl() . '/autocomplete/jquery.autocomplete.css';
    ...
    protected function getClientOptions () {
        ...
        foreach ($properties as $property) {
            if ($this->$property !== null) $options[$property] = $this->$property;
        }
