        ...
        if (isset( $_GET[self::REFRESH_GET_VAR] )) // AJAX request for regenerating code
{
    ...
    public function validate ($input, $caseSensitive) {
        ...
        $valid = $caseSensitive ? ($input === $code) : !strcasecmp( 
                $input , 
                $code );
        $session = Yii::app()->session;
        $session->open();
        ...
        if ($session[$name] > $this->testLimit && $this->testLimit > 0) $this->getVerifyCode( 
                true );
        return $valid;
    }
    ...
    protected function getSessionKey () {
        return self::SESSION_VAR_PREFIX . Yii::app()->getId() . '.' . $this->getController()
            ->getUniqueId() . '.' . $this->getId();
    }
    ...
    protected function renderImage ($code) {
        ...
        imagecolordeallocate( $image , 
                $backColor );
        if ($this->transparent) imagecolortransparent( 
                $image , 
                $backColor );
        ...
        if ($this->fontFile === null) $this->fontFile = dirname( 
                __FILE__ ) . '/Duality.ttf';
        $offset = 2;
        $length = strlen( $code );
        $box = imagettfbbox( 30 , 0 , 
                $this->fontFile , 
                $code );
