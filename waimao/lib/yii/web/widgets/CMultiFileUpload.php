...
class CMultiFileUpload extends CWidget {
    ...
    public function run () {
        ...
        if (($id = $this->getId( false )) === null) {
            if (isset( 
                ...
                $id = CHtml::getIdByName( 
                        $name );
        }
        $this->htmlOptions['id'] = $id;
        $this->registerClientScript();
        echo CHtml::fileField( $name , '' , 
                $this->htmlOptions );
    }
    ...
    public function registerClientScript () {
        $id = $this->htmlOptions['id'];
        $mfOptions = array();
        if ($this->accept !== null) $mfOptions['accept'] = $this->accept;
        if ($this->max > 0) $mfOptions['max'] = $this->max;
        $messages = array();
        foreach (array(
            ...
            if ($this->$messageName !== null) $messages[$messageName] = $this->$messageName;
        }
        if ($messages !== array()) $mfOptions['STRING'] = $messages;
        $options = $mfOptions === array() ? '' : CJavaScript::encode( 
                $mfOptions );
        $cs = Yii::app()->getClientScript();
        $cs->registerCoreScript( 'multifile' );
        $cs->registerScript( 
                'Yii.CMultiFileUpload#' . $id , 
                "jQuery(\"#{$id}\").MultiFile({$options});" );
    }
}
