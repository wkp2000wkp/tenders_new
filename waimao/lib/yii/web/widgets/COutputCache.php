...
class COutputCache extends CFilterWidget {
    ...
    public function filter ($filterChain) {
        $this->init();
        if (!$this->getIsContentCached()) $filterChain->run();
        $this->run();
    }
    ...
    public function run () {
        if ($this->getIsContentCached()) {
            ...
            if (is_array( 
                    $this->dependency )) $this->dependency = Yii::createComponent( 
                    $this->dependency );
            $this->_cache
                ->set( 
                    $this->getCacheKey() , 
    ...
    protected function checkContentCache () {
        if ((empty( $this->requestTypes ) || in_array( 
                Yii::app()->getRequest()
                    ->getRequestType() , 
                $this->requestTypes )) && $this->duration > 0 && ($this->_cache = $this->getCache()) !== null) {
            if (($data = $this->_cache
                ->get( 
    ...
    protected function getCacheKey () {
        if ($this->_key !== null)
            ...
            if ($this->varyByRoute) {
                ...
                if (($action = $controller->getAction()) !== null) $key .= $action->getId();
            }
            $key .= '.';
            if ($this->varyBySession) $key .= Yii::app()->getSession()
                ->getSessionID();
            $key .= '.';
            if (is_array( 
                    $this->varyByParam ) && isset( 
                    $this->varyByParam[0] )) {
                $params = array();
            ...
            $key .= '.';
            if ($this->varyByExpression !== null) $key .= $this->evaluateExpression( 
                    $this->varyByExpression );
            $key .= '.';
        ...
        foreach ($this->_actions as $action) {
            ...
            if (method_exists( 
                ...
                call_user_func_array( 
                        ...
                        $action[2] );
            else if ($action[0] === '' && function_exists( 
                    $action[1] ))
                call_user_func_array( 
                        $action[1] , 
                        $action[2] );
