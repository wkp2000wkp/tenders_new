...
class CDbCommand extends CComponent {
    ...
    public function setText ($value) {
        if ($this->_connection->tablePrefix !== null)
            $this->_text = preg_replace( 
                    '/{{(.*?)}}/' , 
                    $this->_connection->tablePrefix . '\1' , 
                    $value );
        else
            $this->_text = $value;
        $this->cancel();
    ...
    public function prepare () {
        if ($this->_statement == null) {
            try {
                ...
                Yii::log( 
                        'Error in preparing SQL: ' . $this->getText() , 
                        CLogger::LEVEL_ERROR , 
        ...
        if ($dataType === null)
            ...
            $this->_statement
                ->bindParam( 
                    ...
                    $dataType , 
                    $length );
        if ($this->_connection->enableParamLogging) $this->_params[$name] = &$value;
        return $this;
    }
        ...
        if ($dataType === null)
            ...
            $this->_statement
                ->bindValue( 
                    ...
                    $value , 
                    $dataType );
        if ($this->_connection->enableParamLogging) $this->_params[$name] = var_export( 
                $value , 
                true );
        return $this;
    }
    ...
    public function execute ($params = array()) {
        if ($this->_connection->enableParamLogging && ($pars = array_merge( 
                $this->_params , 
                $params )) !== array()) {
            $p = array();
            foreach ($pars as $name => $value)
                $p[$name] = $name . '=' . $value;
            $par = '. Bind with parameter ' . implode( 
                    ', ' , 
                    $p );
        }
        else
            $par = '';
        Yii::trace( 
                'Executing SQL: ' . $this->getText() . $par , 
                'system.db.CDbCommand' );
        try {
            if ($this->_connection->enableProfiling) Yii::beginProfile( 
                    'system.db.CDbCommand.execute(' . $this->getText() . ')' , 
            ...
            $n = $this->_statement
                ->rowCount();
            if ($this->_connection->enableProfiling) Yii::endProfile( 
                    'system.db.CDbCommand.execute(' . $this->getText() . ')' , 
                    'system.db.CDbCommand.execute' );
            return $n;
        }
            ...
            if ($this->_connection->enableProfiling) Yii::endProfile( 
                    ...
                    'system.db.CDbCommand.execute' );
            Yii::log( 
                    'Error in executing SQL: ' . $this->getText() . $par , 
                    CLogger::LEVEL_ERROR , 
                    'system.db.CDbCommand' );
            $errorInfo = $e instanceof PDOException ? $e->errorInfo : null;
            throw new CDbException( 
    ...
    public function queryAll ($fetchAssociative = true, $params = array()) {
        return $this->queryInternal( 'fetchAll' , 
                $fetchAssociative ? PDO::FETCH_ASSOC : PDO::FETCH_NUM , 
                $params );
    }
    ...
    public function queryScalar ($params = array()) {
        $result = $this->queryInternal( 
                ...
                0 , 
                $params );
        if (is_resource( $result ) && get_resource_type( 
                $result ) === 'stream')
    ...
    private function queryInternal ($method, $mode, $params = array()) {
        if ($this->_connection->enableParamLogging && ($pars = array_merge( 
            ...
            $par = '';
        Yii::trace( 
                'Querying SQL: ' . $this->getText() . $par , 
                'system.db.CDbCommand' );
        try {
            if ($this->_connection->enableProfiling) Yii::beginProfile( 
                    'system.db.CDbCommand.query(' . $this->getText() . ')' , 
            ...
            if ($this->_connection->enableProfiling) Yii::endProfile( 
                    ...
                    'system.db.CDbCommand.query' );
            Yii::log( 
                    'Error in querying SQL: ' . $this->getText() . $par , 
                    CLogger::LEVEL_ERROR , 
                    'system.db.CDbCommand' );
            $errorInfo = $e instanceof PDOException ? $e->errorInfo : null;
            throw new CDbException( 
