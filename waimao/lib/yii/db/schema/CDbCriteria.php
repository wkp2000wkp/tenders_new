...
class CDbCriteria extends CComponent {
    ...
    public function addCondition ($condition, $operator = 'AND') {
        if (is_array( $condition )) {
            if ($condition === array()) return $this;
            $condition = '(' . implode( 
                    ') ' . $operator . ' (' , 
                    $condition ) . ')';
        }
    ...
    public function addInCondition ($column, $values, $operator = 'AND') {
        if (($n = count( $values )) < 1) return $this->addCondition( 
                '0=1' , 
                $operator ); // 0=1 is used because in MSSQL value alone can't be used in WHERE
        if ($n === 1) {
            $value = reset( 
                    $values );
            if ($value === null) return $this->addCondition( 
                    $column . ' IS NULL' );
    ...
    public function addNotInCondition ($column, $values, $operator = 'AND') {
        if (($n = count( $values )) < 1) return $this;
        if ($n === 1) {
            $value = reset( 
                    $values );
            if ($value === null) return $this->addCondition( 
            ...
            $condition = $column . ' NOT IN (' . implode( 
                    ', ' , 
                    $params ) . ')';
        }
        return $this->addCondition( $condition , 
    ...
    public function compare ($column, $value, $partialMatch = false, $operator = 'AND') {
        ...
        if ($partialMatch) {
            if ($op === '') return $this->addSearchCondition( 
                    $column , 
                    $value , 
                    true , 
                    $operator );
        ...
        $this->addCondition( 
                $column . $op . self::PARAM_PREFIX . self::$paramCount , 
                $operator );
        $this->params[self::PARAM_PREFIX . self::$paramCount++] = $value;
        return $this;
    }
    ...
    public function addBetweenCondition ($column, $valueStart, $valueEnd, $operator = 'AND') {
        if ($valueStart === '' || $valueEnd === '') return $this;
    ...
    public function mergeWith ($criteria, $useAnd = true) {
        $and = $useAnd ? 'AND' : 'OR';
        if (is_array( $criteria )) $criteria = new self( 
                $criteria );
        ...
        if ($this->params !== $criteria->params) $this->params = array_merge( 
                $this->params , 
                $criteria->params );
        if ($criteria->limit > 0) $this->limit = $criteria->limit;
        if ($criteria->offset >= 0) $this->offset = $criteria->offset;
        if ($criteria->alias !== null) $this->alias = $criteria->alias;
        ...
        if ($criteria->distinct > 0) $this->distinct = $criteria->distinct;
        if ($criteria->together !== null) $this->together = $criteria->together;
        if (empty( $this->with ))
            $this->with = $criteria->with;
        else if (!empty( $criteria->with )) $this->with = CMap::mergeArray( 
                $this->with , 
                $criteria->with );
    }
