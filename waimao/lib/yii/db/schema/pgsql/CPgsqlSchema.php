...
class CPgsqlSchema extends CDbSchema {
    ...
    protected function createTable ($name) {
        ...
        if (!$this->findColumns( $table )) return null;
        $this->findConstraints( $table );
        if (is_string( $table->primaryKey ) && isset( 
                $this->_sequences[$table->primaryKey] ))
    ...
    protected function resolveTableNames ($table, $name) {
        ...
        if ($schemaName === self::DEFAULT_SCHEMA)
            ...
            $table->rawName = $this->quoteTableName( 
                    $schemaName ) . '.' . $this->quoteTableName( 
                    $tableName );
    }
...
EOD;
        ...
        $command->bindValue( ':schema' , 
                $table->schemaName );
        if (($columns = $command->queryAll()) === array()) return false;
        foreach ($columns as $column) {
            ...
            if (stripos( 
                    $column['adsrc'] , 
                    'nextval' ) === 0 && preg_match( 
                    '/nextval\([^\']*\'([^\']+)\'[^\)]*\)/i' , 
                    $column['adsrc'] , 
                    $matches )) {
                if (strpos( 
                        $matches[1] , 
                        '.' ) !== false || $table->schemaName === self::DEFAULT_SCHEMA)
    ...
    protected function createColumn ($column) {
        ...
        $c->init( $column['type'] , 
                $column['atthasdef'] ? $column['adsrc'] : null );
        return $c;
    }
    ...
    protected function findForeignKey ($table, $src) {
        ...
        if (preg_match( $pattern , 
            ...
            foreach ($keys as $i => $key) {
                $table->foreignKeys[$key] = array(
                    
                        $tableName, 
                        $fkeys[$i]
                );
                if (isset( 
                        $table->columns[$key] )) $table->columns[$key]->isForeignKey = true;
            }
        }
    }
