...
class CLogger extends CComponent {
    ...
    public function log ($message, $level = 'info', $category = 'application') {
        ...
        $this->_logCount++;
        if ($this->autoFlush > 0 && $this->_logCount >= $this->autoFlush) $this->flush();
    }
    ...
    public function getLogs ($levels = '', $categories = '') {
        $this->_levels = preg_split( '/[\s,]+/' , 
                strtolower( 
                        $levels ) , 
                -1 , 
                PREG_SPLIT_NO_EMPTY );
        $this->_categories = preg_split( 
                '/[\s,]+/' , 
                strtolower( 
                        $categories ) , 
                -1 , 
                PREG_SPLIT_NO_EMPTY );
        if (empty( $levels ) && empty( 
                $categories ))
            return $this->_logs;
    ...
    public function getProfilingResults ($token = null, $category = null, $refresh = false) {
        if ($this->_timings === null || $refresh) $this->calculateTimings();
        if ($token === null && $category === null) return $this->_timings;
        $results = array();
        foreach ($this->_timings as $timing) {
            if (($category === null || $timing[1] === $category) && ($token === null || $timing[0] === $token)) $results[] = $timing[2];
        }
        return $results;
    }
    ...
    private function calculateTimings () {
        ...
        foreach ($this->_logs as $log) {
            if ($log[1] !== CLogger::LEVEL_PROFILE) continue;
