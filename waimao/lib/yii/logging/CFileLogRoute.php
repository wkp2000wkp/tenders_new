...
class CFileLogRoute extends CLogRoute {
    ...
    public function init () {
        parent::init();
        if ($this->getLogPath() === null) $this->setLogPath( 
    ...
    public function setLogPath ($value) {
        ...
        if ($this->_logPath === false || !is_dir( 
                $this->_logPath ) || !is_writable( 
                $this->_logPath )) throw new CException( 
                Yii::t( 
                        'yii' , 
    ...
    public function setMaxFileSize ($value) {
        if (($this->_maxFileSize = (int) $value) < 1) $this->_maxFileSize = 1;
    }
    ...
    public function setMaxLogFiles ($value) {
        if (($this->_maxLogFiles = (int) $value) < 1) $this->_maxLogFiles = 1;
    }
    ...
    protected function processLogs ($logs) {
        ...
        if (@filesize( $logFile ) > $this->getMaxFileSize() * 1024) $this->rotateFiles();
        foreach ($logs as $log)
            error_log( 
