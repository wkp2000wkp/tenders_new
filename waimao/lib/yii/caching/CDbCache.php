...
EOD;
    ...
    protected function getDbConnection () {
        if ($this->_db !== null)
            ...
            if (($this->_db = Yii::app()->getComponent( 
                    $id )) instanceof CDbConnection)
                return $this->_db;
            else
            ...
            return $this->_db = new CDbConnection( 
                    'sqlite:' . $dbFile );
        }
    }
    ...
    protected function gc () {
        $this->getDbConnection()
            ->createCommand( 
                "DELETE FROM {$this->cacheTableName} WHERE expire>0 AND expire<" . time() )
            ->execute();
    }
    ...
    public function flush () {
        $this->getDbConnection()
            ->createCommand( 
                "DELETE FROM {$this->cacheTableName}" )
            ->execute();
        return true;
    }
}
