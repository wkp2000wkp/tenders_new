...
class CStatePersister extends CApplicationComponent implements IStatePersister {
    ...
    public function init () {
        parent::init();
        if ($this->stateFile === null) $this->stateFile = Yii::app()->getRuntimePath() . DIRECTORY_SEPARATOR . 'state.bin';
        $dir = dirname( $this->stateFile );
        if (!is_dir( $dir ) || !is_writable( 
                $dir )) throw new CException( 
                Yii::t( 
                        'yii' , 
    ...
    public function load () {
        ...
        if ($this->cacheID !== false && ($cache = Yii::app()->getComponent( 
            ...
            if (($value = $cache->get( 
                ...
                return unserialize( 
                        $value );
            else if (($content = @file_get_contents( 
                    $stateFile )) !== false) {
                $cache->set( 
                        $cacheKey , 
                        $content , 
                        0 , 
                        new CFileCacheDependency( 
                                $stateFile ) );
                return unserialize( 
                        $content );
            }
