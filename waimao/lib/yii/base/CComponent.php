...
class CComponent {
    ...
    public function __get ($name) {
        ...
        if (method_exists( $this , $getter ))
            return $this->$getter();
        else if (strncasecmp( $name , 'on' , 2 ) === 0 && method_exists( 
                $this , 
                $name )) {
            ...
            $name = strtolower( 
                    $name );
            if (!isset( 
                    $this->_e[$name] )) $this->_e[$name] = new CList();
            return $this->_e[$name];
        }
            ...
            foreach ($this->_m as $object) {
                if ($object->getEnabled() && (property_exists( 
                        $object , 
                        $name ) || $object->canGetProperty( 
    ...
    public function __set ($name, $value) {
        ...
        if (method_exists( $this , $setter ))
            ...
            foreach ($this->_m as $object) {
                if ($object->getEnabled() && (property_exists( 
                        $object , 
                        $name ) || $object->canSetProperty( 
                        $name ))) return $object->$name = $value;
            }
        }
            ...
            if (isset( 
                ...
                foreach ($this->_m as $object) {
                    if ($object->getEnabled()) {
                        ...
                        else if ($object->canSetProperty( 
                                $name )) return $object->$setter( 
                                null );
                    }
                }
            }
    ...
    public function __call ($name, $parameters) {
        if ($this->_m !== null) {
            foreach ($this->_m as $object) {
                if ($object->getEnabled() && method_exists( 
                        $object , 
        ...
        if (class_exists( 'Closure' , false ) && $this->$name instanceof Closure) return call_user_func_array( 
                $this->$name , 
                $parameters );
        throw new CException( 
                Yii::t( 
    ...
    public function attachBehavior ($name, $behavior) {
        if (!($behavior instanceof IBehavior)) $behavior = Yii::createComponent( 
                $behavior );
    ...
    public function enableBehavior ($name) {
        if (isset( $this->_m[$name] )) $this->_m[$name]
            ->setEnabled( true );
    }
    ...
    public function hasEvent ($name) {
        return !strncasecmp( $name , 'on' , 2 ) && method_exists( 
                $this , 
                $name );
    }
    ...
    public function hasEventHandler ($name) {
        ...
        return isset( $this->_e[$name] ) && $this->_e[$name]
            ->getCount() > 0;
    }
    ...
    public function raiseEvent ($name, $event) {
        ...
        if (isset( $this->_e[$name] )) {
            foreach ($this->_e[$name] as $handler) {
                ...
                if (($event instanceof CEvent) && $event->handled) return;
            }
        }
        else if (YII_DEBUG && !$this->hasEvent( 
                $name )) throw new CException( 
                Yii::t( 
                        'yii' , 
                        'Event "{class}.{event}" is not defined.' , 
                        array(
