...
<td>&nbsp;</td>
<td>
	<td><a href='<?php echo URL_DOMAIN;?>/index.php?r=backend/up&tb_id=<?php echo $tou_biao_id; ?>&referer=<?php echo urlencode($referer); ?>'  class='ui-state-default ui-corner-all'>【上传文件】</a> |
...
<script>
	function deleteUploadFile(){
		if(confirm("确定删除?")){
	    	$("#ZBForm").attr('action','<?php echo URL_DOMAIN; ?>/index.php?r=operation/oufr&action=delete&tb_id=<?php echo $tou_biao_id; ?>');
	    	$("#ZBForm").submit();
		}else{
			return false;
		}
