...
{ /*{{{*/
    ...
    function setNullOption ($nullOption) { /*{{{*/
        if ($nullOption == STATS_REJECT_NULL || $nullOption == STATS_IGNORE_NULL || $nullOption == STATS_USE_NULL_AS_ZERO) {
            $this->_nullOption = $nullOption;
            return true;
        }
        else {
            return PEAR::raiseError( 'invalid null handling option expecting: ' . 'STATS_REJECT_NULL, STATS_IGNORE_NULL or STATS_USE_NULL_AS_ZERO' );
        }
    } /*}}}*/
    ...
    function harmonicMean () { /*{{{*/
        ...
        if (!array_key_exists( 'harmonicMean' , $this->_calculatedValues )) {
            ...
            if ($this->_dataOption == STATS_DATA_CUMMULATIVE) {
                foreach ($this->_data as $val => $freq) {
                    if ($val == 0) {
                        return PEAR::raiseError( 'cannot calculate a ' . 'harmonic mean with data values of zero.' );
                    }
                    $invsum += $freq / $val;
                }
            }
    ...
    function coeffOfVariation () { /*{{{*/
        if (!array_key_exists( 'coeffOfVariation' , $this->_calculatedValues )) {
            ...
            if ($mean == 0.0) {
                return PEAR::raiseError( 'cannot calculate the coefficient ' . 'of variation, mean of sample is zero' );
            }
            $stDev = $this->stDev();
            if (PEAR::isError( $stDev )) {
                return $stDev;
    ...
    function interquartileMean () { /*{{{*/
        if (!array_key_exists( 'interquartileMean' , $this->_calculatedValues )) {
            ...
            if ($n == 0) {
                return PEAR::raiseError( 'error calculating interquartile mean, ' . 'empty interquartile range of values.' );
            }
