...
class PHPExcel_Reader_Excel5 implements PHPExcel_Reader_IReader
{
    ...
    public function load ($pFilename) {
        ...
        $this->_phpExcel
            ->removeSheetByIndex( 0 ); // remove 1st sheet
        if (!$this->_readDataOnly) {
            $this->_phpExcel
                ->removeCellStyleXfByIndex( 0 ); // remove the default style
            $this->_phpExcel
                ->removeCellXfByIndex( 0 ); // remove the default style
        }
        ...
        if (!$this->_readDataOnly) {
            ...
            foreach ($this->_phpExcel
                ...
                // border colors
                $top = $objStyle->getBorders()
                    ->getTop();
                $right = $objStyle->getBorders()
                    ->getRight();
                $bottom = $objStyle->getBorders()
                    ->getBottom();
                $left = $objStyle->getBorders()
                    ->getLeft();
                $diagonal = $objStyle->getBorders()
                    ->getDiagonal();
        ...
        foreach ($this->_sheets as $sheet) {
            ...
            if (!$this->_readDataOnly && $this->_drawingData) {
                ...
                $allSpContainers = $escherWorksheet->getDgContainer()
                    ->getSpgrContainer()
                    ->getAllSpContainers();
            }
            // treat OBJ records
            foreach ($this->_objs as $n => $obj) {
                ...
                switch ($obj['type']) {
                    case 0x08:
                        ...
                        $BSECollection = $escherWorkbook->getDggContainer()
                            ->getBstoreContainer()
                            ->getBSECollection();
                    ...
                    case pack( 'C' , 0x06 ):
                        ...
                        if ($docSheet = $this->_phpExcel
                            ...
                            $docSheet->getPageSetup()
                                ->setPrintArea( implode( ',' , $extractedRanges ) ); // C7:J66,A1:IV2
                        }
                        break;
                    case pack( 'C' , 0x07 ):
    ...
    private function _readXf () {
        ...
        if (!$this->_readDataOnly) {
            ...
            if ($this->_version == self::XLS_BIFF8) {
                ...
                $objStyle->getBorders()
                    ->getTop()->colorIndex = (0x0000007F & $this->_GetInt4d( $recordData , 14 )) >> 0;
                // bit: 13-7; mask: 0x00003F80; bottom color
                $objStyle->getBorders()
                    ->getBottom()->colorIndex = (0x00003F80 & $this->_GetInt4d( $recordData , 14 )) >> 7;
                // bit: 20-14; mask: 0x001FC000; diagonal color
                $objStyle->getBorders()
                    ->getDiagonal()->colorIndex = (0x001FC000 & $this->_GetInt4d( $recordData , 14 )) >> 14;
                // bit: 24-21; mask: 0x01E00000; diagonal style
                if ($bordersDiagonalStyle = $this->_mapBorderStyle( (0x01E00000 & $this->_GetInt4d( $recordData , 14 )) >> 21 )) {
                    $objStyle->getBorders()
                        ->getDiagonal()
                        ->setBorderStyle( $bordersDiagonalStyle );
                }
                ...
                $objStyle->getFill()
                    ->setFillType( $this->_mapFillPattern( (0x003F0000 & $borderAndBackground) >> 16 ) );
                // bit: 24-22; mask: 0x01C00000; bottom line style
                $objStyle->getBorders()
                    ->getBottom()
                    ->setBorderStyle( $this->_mapBorderStyle( (0x01C00000 & $borderAndBackground) >> 22 ) );
                ...
                $objStyle->getBorders()
                    ->getTop()
                    ->setBorderStyle( $this->_mapBorderStyle( (0x00000007 & $borderLines) >> 0 ) );
                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()
                    ->getLeft()
                    ->setBorderStyle( $this->_mapBorderStyle( (0x00000038 & $borderLines) >> 3 ) );
                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()
                    ->getRight()
                    ->setBorderStyle( $this->_mapBorderStyle( (0x000001C0 & $borderLines) >> 6 ) );
                // bit: 15-9; mask: 0x0000FE00; top line color index
                $objStyle->getBorders()
                    ->getTop()->colorIndex = (0x0000FE00 & $borderLines) >> 9;
                // bit: 22-16; mask: 0x007F0000; left line color index
                $objStyle->getBorders()
                    ->getLeft()->colorIndex = (0x007F0000 & $borderLines) >> 16;
        ...
        if ($this->_version == self::XLS_BIFF8) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                $this->_ref[] = array( // offset: 2 + 6 * $i; index to EXTERNALBOOK record
                        ...
                        'firstSheetIndex' => $this->_GetInt2d( $recordData , 4 + 6 * $i ),  // offset: 6 + 6 * $i; index to last sheet in EXTERNALBOOK record
    ...
    private function _readHorizontalPageBreaks () {
        ...
        if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->_phpSheet
                    ->setBreakByColumnAndRow( $cf , $r , PHPExcel_Worksheet::BREAK_ROW );
            }
        }
    }
    ...
    private function _readHcenter () {
        ...
        if (!$this->_readDataOnly) {
            ...
            $this->_phpSheet
                ->getPageSetup()
                ->setHorizontalCentered( $isHorizontalCentered );
        }
    }
    ...
    private function _readVcenter () {
        ...
        if (!$this->_readDataOnly) {
            ...
            $this->_phpSheet
                ->getPageSetup()
                ->setVerticalCentered( $isVerticalCentered );
        }
    }
    ...
    private function _readLeftMargin () {
        ...
        if (!$this->_readDataOnly) {
            ...
            $this->_phpSheet
                ->getPageMargins()
                ->setLeft( $this->_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readRightMargin () {
        ...
        if (!$this->_readDataOnly) {
            ...
            $this->_phpSheet
                ->getPageMargins()
                ->setRight( $this->_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readTopMargin () {
        ...
        if (!$this->_readDataOnly) {
            ...
            $this->_phpSheet
                ->getPageMargins()
                ->setTop( $this->_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readBottomMargin () {
        ...
        if (!$this->_readDataOnly) {
            ...
            $this->_phpSheet
                ->getPageMargins()
                ->setBottom( $this->_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readRk () {
        ...
        // Read cell?
        if (!is_null( $this->getReadFilter() ) && $this->getReadFilter()
    ...
    private function _readWindow2 () {
        ...
        $this->_phpSheet
            ->setRightToLeft( (bool) ((0x0040 & $options) >> 6) );
    ...
    private function _readPane () {
        ...
        if (!$this->_readDataOnly) {
            ...
            if ($this->_frozen) {
                // frozen panes
                $this->_phpSheet
                    ->freezePane( PHPExcel_Cell::stringFromColumnIndex( $px ) . ($py + 1) );
            }
            else {
