...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    public function calculateColumnWidths ($calculateMergeCells = false) {
        ...
        if (!empty( $autoSizes )) {
            ...
            foreach ($this->getCellCollection( false ) as $cellID) {
                ...
                if (isset( $autoSizes[$cell->getColumn()] )) {
                    ...
                    if (!isset( $isMergeCell[$cell->getCoordinate()] )) {
                        ...
                        // To formatted string
                        $cellValue = PHPExcel_Style_NumberFormat::toFormattedString( $cellValue , $this->getParent()
                            ->getCellXfByIndex( $cell->getXfIndex() )
                            ->getNumberFormat()
                            ->getFormatCode() );
                        $autoSizes[$cell->getColumn()] = max( (float) $autoSizes[$cell->getColumn()] , (float) PHPExcel_Shared_Font::calculateColumnWidth( $this->getParent()
                            ->getCellXfByIndex( $cell->getXfIndex() )
                            ->getFont() , $cellValue , $this->getParent()
                            ->getCellXfByIndex( $cell->getXfIndex() )
                            ->getAlignment()
                            ->getTextRotation() , $this->getDefaultStyle()
                            ->getFont() ) );
                    }
                }
            }
            ...
            foreach ($autoSizes as $columnIndex => $width) {
                if ($width == -1) $width = $this->getDefaultColumnDimension()
                    ->getWidth();
    ...
    public function rebindParent (PHPExcel $parent) {
        $namedRanges = $this->_parent
            ->getNamedRanges();
        foreach ($namedRanges as $namedRange) {
            $parent->addNamedRange( $namedRange );
        }
        $this->_parent
            ->removeSheetByIndex( $this->_parent
            ->getIndex( $this ) );
        $this->_parent = $parent;
        return $this;
    }
    ...
    public function setTitle ($pValue = 'Worksheet') {
        ...
        if ($this->getParent()
            ->getSheetByName( $pValue )) {
            // Use name, but append with lowest possible integer
            $i = 1;
            while ($this->getParent()
                ->getSheetByName( $pValue . ' ' . $i )) {
                ++$i;
            }
            $altTitle = $pValue . ' ' . $i;
    ...
    public function getCell ($pCoordinate = 'A1') {
        ...
        if ((!preg_match( '/^' . PHPExcel_Calculation::CALCULATION_REGEXP_CELLREF . '$/i' , $pCoordinate , $matches )) && (preg_match( '/^' . PHPExcel_Calculation::CALCULATION_REGEXP_NAMEDRANGE . '$/i' , $pCoordinate , $matches ))) {
            ...
            if (!is_null( $namedRange )) {
                ...
                return $namedRange->getWorksheet()
                    ->getCell( $pCoordinate );
            }
        }
        ...
        elseif (strpos( $pCoordinate , '$' ) !== false) {
            ...
            $cell = $this->_cellCollection
                ->addCacheData( $pCoordinate , new PHPExcel_Cell( $aCoordinates[0] , $aCoordinates[1] , null , PHPExcel_Cell_DataType::TYPE_NULL , $this ) );
            $this->_cellCollectionIsSorted = false;
            if (PHPExcel_Cell::columnIndexFromString( $this->_cachedHighestColumn ) < PHPExcel_Cell::columnIndexFromString( $aCoordinates[0] )) $this->_cachedHighestColumn = $aCoordinates[0];
            if ($this->_cachedHighestRow < $aCoordinates[1]) $this->_cachedHighestRow = $aCoordinates[1];
    ...
    public function getCellByColumnAndRow ($pColumn = 0, $pRow = 0) {
        ...
        if (!$this->_cellCollection
            ...
            $cell = $this->_cellCollection
                ->addCacheData( $coordinate , new PHPExcel_Cell( $columnLetter , $pRow , null , PHPExcel_Cell_DataType::TYPE_NULL , $this ) );
            $this->_cellCollectionIsSorted = false;
            if (PHPExcel_Cell::columnIndexFromString( $this->_cachedHighestColumn ) < $pColumn) $this->_cachedHighestColumn = $columnLetter;
            if ($this->_cachedHighestRow < $pRow) $this->_cachedHighestRow = $pRow;
            return $cell;
        }
        return $this->_cellCollection
    ...
    public function cellExists ($pCoordinate = 'A1') {
        ...
        if (strpos( $pCoordinate , '!' ) !== false) {
            ...
            return $this->getParent()
                ->getSheetByName( $worksheetReference[0] )
                ->cellExists( $worksheetReference[1] );
        }
        // Named range?
        if ((!preg_match( '/^' . PHPExcel_Calculation::CALCULATION_REGEXP_CELLREF . '$/i' , $pCoordinate , $matches )) && (preg_match( '/^' . PHPExcel_Calculation::CALCULATION_REGEXP_NAMEDRANGE . '$/i' , $pCoordinate , $matches ))) {
            $namedRange = PHPExcel_NamedRange::resolveRange( $pCoordinate , $this );
            if (!is_null( $namedRange )) {
                $pCoordinate = $namedRange->getRange();
                if ($this->getHashCode() != $namedRange->getWorksheet()
                    ->getHashCode()) {
    ...
    public function getColumnDimension ($pColumn = 'A') {
        ...
        if (!isset( $this->_columnDimensions[$pColumn] )) {
            ...
            if (PHPExcel_Cell::columnIndexFromString( $this->_cachedHighestColumn ) < PHPExcel_Cell::columnIndexFromString( $pColumn )) $this->_cachedHighestColumn = $pColumn;
        }
    ...
    public function duplicateStyle (PHPExcel_Style $pCellStyle = null, $pRange = '') {
        ...
        if ($existingStyle = $this->_parent
            ->getCellXfByHashCode( $pCellStyle->getHashCode() )) {
        ...
        for ($row = $dimension[0][1]; $row <= $dimension[1][1]; ++$row) {
            for ($column = $dimension[0][0]; $column <= $dimension[1][0]; ++$column) {
                ...
                if ($this->cellExistsByColumnAndRow( $column , $row )) {
                    ...
                    $style = $this->_parent
                        ->getCellXfByIndex( $cell->getXfIndex() );
                    $returnValue[$row][$column] = PHPExcel_Style_NumberFormat::toFormattedString( $returnValue[$row][$column] , $style->getNumberFormat()
                        ->getFormatCode() );
                }
                else {
    ...
    public function garbageCollect () {
        ...
        $iterator = $this->getDrawingCollection()
            ->getIterator();
        while ($iterator->valid()) {
            $imageCoordinates[$iterator->current()
                ->getCoordinates()] = true;
            $iterator->next();
        }
    ...
    public function getHashCode () {
        return md5( $this->_title . $this->_autoFilter . ($this->_protection
            ->isProtectionEnabled() ? 't' : 'f') . //. $this->calculateWorksheetDimension()
__CLASS__ );
    }
    ...
    public function getTabColor () {
        if (is_null( $this->_tabColor )) $this->_tabColor = new PHPExcel_Style_Color();
