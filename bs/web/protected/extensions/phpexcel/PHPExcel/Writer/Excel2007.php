    ...
    public function save ($pFilename = null) {
        if (!is_null( $this->_spreadSheet )) {
            ...
            $this->_stylesConditionalHashTable
                ->addFromSource( $this->getWriterPart( 'Style' )
            ...
            $this->_drawingHashTable
                ->addFromSource( $this->getWriterPart( 'Drawing' )
            ...
            for ($i = 0; $i < $this->_spreadSheet
                ->getSheetCount(); ++$i) {
                // Add relationships
                $objZip->addFromString( 'xl/worksheets/_rels/sheet' . ($i + 1) . '.xml.rels' , $this->getWriterPart( 'Rels' )
                ...
                if ($this->_spreadSheet
                    ...
                    $objZip->addFromString( 'xl/drawings/_rels/drawing' . ($i + 1) . '.xml.rels' , $this->getWriterPart( 'Rels' )
                        ->writeDrawingRelationships( $this->_spreadSheet
                        ->getSheet( $i ) ) );
                    // Drawings
                    $objZip->addFromString( 'xl/drawings/drawing' . ($i + 1) . '.xml' , $this->getWriterPart( 'Drawing' )
                ...
                if (count( $this->_spreadSheet
                    ...
                    ->getComments() ) > 0) {
                    // VML Comments
                    $objZip->addFromString( 'xl/drawings/vmlDrawing' . ($i + 1) . '.vml' , $this->getWriterPart( 'Comments' )
                ...
                if (count( $this->_spreadSheet
                    ...
                    ->getImages() ) > 0) {
                    // VML Drawings
                    $objZip->addFromString( 'xl/drawings/vmlDrawingHF' . ($i + 1) . '.vml' , $this->getWriterPart( 'Drawing' )
                        ->writeVMLHeaderFooterImages( $this->_spreadSheet
                        ->getSheet( $i ) ) );
                    // VML Drawing relationships
                    $objZip->addFromString( 'xl/drawings/_rels/vmlDrawingHF' . ($i + 1) . '.vml.rels' , $this->getWriterPart( 'Rels' )
            ...
            // Add media
            for ($i = 0; $i < $this->getDrawingHashTable()
                ->count(); ++$i) {
                if ($this->getDrawingHashTable()
                    ->getByIndex( $i ) instanceof PHPExcel_Worksheet_Drawing) {
                    $imageContents = null;
                    $imagePath = $this->getDrawingHashTable()
                        ->getByIndex( $i )
                        ->getPath();
                    ...
                    ob_start();
                    call_user_func( $this->getDrawingHashTable()
                        ->getByIndex( $i )
