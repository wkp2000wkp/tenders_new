...
class PHPExcel_Writer_Excel5 implements PHPExcel_Writer_IWriter
{
    ...
    public function save ($pFilename = null) {
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            $this->_writerWorksheets[$i] = new PHPExcel_Writer_Excel5_Worksheet( $this->_BIFF_version , $this->_str_total , $this->_str_unique , $this->_str_table , $this->_colors , $this->_parser , $this->_preCalculateFormulas , $this->_phpExcel
                ->getSheet( $i ) );
        }
        ...
        $OLE->append( $this->_writerWorkbook
            ->writeWorkbook( $worksheetSizes ) );
        // add binary data for sheet streams
        for ($i = 0; $i < $countSheets; ++$i) {
            $OLE->append( $this->_writerWorksheets[$i]
                ->getData() );
        }
        $root = new PHPExcel_Shared_OLE_PPS_Root( time() , time() , array(
            $OLE
        ) );
        // save the OLE file
    ...
    private function _buildWorksheetEschers () {
        ...
        foreach ($this->_phpExcel
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                $spId = $reducedSpId | ($sheet->getParent()
                    ->getIndex( $sheet ) + 1) << 10;
