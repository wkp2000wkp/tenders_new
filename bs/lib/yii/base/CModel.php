 ...
 * @since 1.0
 */
abstract class CModel extends CComponent implements 
    ...
    public function createValidators () {
        ...
        foreach ($this->rules() as $rule) {
            if (isset( $rule[0] , 
                    $rule[1] )) // attributes, validator name
                $validators->add( 
    ...
    public function getError ($attribute) {
        return isset( $this->_errors[$attribute] ) ? reset( 
    ...
    public function setAttributes ($values, $safeOnly = true) {
        ...
        foreach ($values as $name => $value) {
            if (isset( 
                    $attributes[$name] ))
                $this->$name = $value;
            else if ($safeOnly) $this->onUnsafeAttribute( 
                    $name , 
                    $value );
        }
    }
    ...
    public function unsetAttributes ($names = null) {
        if ($names === null) $names = $this->attributeNames();
        foreach ($names as $name)
            $this->$name = null;
    }
