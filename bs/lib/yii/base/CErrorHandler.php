 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CErrorHandler.php 2426 2010-09-05 12:51:26Z qiang.xue $
 * @package system.base
 * @since 1.0
 */
class CErrorHandler extends CApplicationComponent {
    ...
    protected function handleException ($exception) {
        ...
        if ($app instanceof CWebApplication) {
            ...
            $this->_error = $data = array(
                    ...
                    'source' => $this->getSourceLines( 
                            $fileName , 
                            $errorLine )
            );
            if (!headers_sent()) header( 
                    "HTTP/1.0 {$data['code']} " . get_class( 
                            $exception ) );
    ...
    protected function handleError ($event) {
        ...
        foreach ($trace as $i => $t) {
            ...
            if (isset( 
                    $t['object'] ) && is_object( 
                    $t['object'] )) $traceString .= get_class( 
                    $t['object'] ) . '->';
    ...
    protected function getExactTrace ($exception) {
        ...
        foreach ($traces as $trace) {
            ...
            if (isset( 
                    $trace['function'] ) && ($trace['function'] === '__get' || $trace['function'] === '__set')) return $trace;
        }
        return null;
    }
    ...
    protected function getViewFile ($view, $code) {
        $viewPaths = array(
                ...
                Yii::app()->getTheme() === null ? null : Yii::app()->getTheme()
                    ->getSystemViewPath(), 
        ...
        foreach ($viewPaths as $i => $viewPath) {
            if ($viewPath !== null) {
                $viewFile = $this->getViewFileInternal( 
                        $viewPath , 
                        $view , 
                        $code , 
                        $i === 2 ? 'en_us' : null );
                if (is_file( 
                        $viewFile )) return $viewFile;
            }
        }
    }
    ...
    protected function getViewFileInternal ($viewPath, $view, $code, $srcLanguage = null) {
        ...
        if ($view === 'error') {
            $viewFile = $app->findLocalizedFile( 
                    $viewPath . DIRECTORY_SEPARATOR . "error{$code}.php" , 
                    $srcLanguage );
            if (!is_file( 
                    $viewFile )) $viewFile = $app->findLocalizedFile( 
                    $viewPath . DIRECTORY_SEPARATOR . 'error.php' , 
                    $srcLanguage );
        }
        else
            $viewFile = $app->findLocalizedFile( 
                    $viewPath . DIRECTORY_SEPARATOR . "exception.php" , 
                    $srcLanguage );
        return $viewFile;
    }
    ...
    protected function getVersionInfo () {
        if (YII_DEBUG) {
            ...
            if (isset( 
                    $_SERVER['SERVER_SOFTWARE'] )) $version = $_SERVER['SERVER_SOFTWARE'] . ' ' . $version;
        }
        else
            $version = '';
        return $version;
    ...
    protected function getSourceLines ($file, $line) {
        ...
        if ($line < 0 || ($lines = @file( 
                $file )) === false || ($lineCount = count( 
                $lines )) <= $line) return array();
