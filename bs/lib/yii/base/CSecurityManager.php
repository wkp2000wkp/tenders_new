...
class CSecurityManager extends CApplicationComponent {
    ...
    public function getValidationKey () {
        if ($this->_validationKey !== null)
            ...
            if (($key = Yii::app()->getGlobalState( 
                    self::STATE_VALIDATION_KEY )) !== null)
    ...
    public function getEncryptionKey () {
        if ($this->_encryptionKey !== null)
            ...
            if (($key = Yii::app()->getGlobalState( 
                    self::STATE_ENCRYPTION_KEY )) !== null)
    ...
    public function encrypt ($data, $key = null) {
        ...
        $iv = mcrypt_create_iv( 
                mcrypt_enc_get_iv_size( 
                        $module ) , 
                MCRYPT_RAND );
        mcrypt_generic_init( $module , $key , 
                $iv );
        $encrypted = $iv . mcrypt_generic( 
                $module , 
                $data );
    ...
    public function decrypt ($data, $key = null) {
        ...
        mcrypt_generic_init( $module , $key , 
                $iv );
        $decrypted = mdecrypt_generic( $module , 
                substr( 
                        $data , 
                        $ivSize ) );
        mcrypt_generic_deinit( $module );
        ...
        if (extension_loaded( 'mcrypt' )) {
            if (is_array( 
                ...
                $module = mcrypt_module_open( 
                        ...
                        MCRYPT_MODE_CBC , 
                        '' );
            if ($module === false) throw new CException( 
                    Yii::t( 
                            'yii' , 
    ...
    protected function computeHMAC ($data, $key = null) {
        if ($key === null) $key = $this->getValidationKey();
        if (function_exists( 'hash_hmac' )) return hash_hmac( 
                $this->hashAlgorithm , 
                $data , 
                $key );
        if (!strcasecmp( $this->hashAlgorithm , 
                'sha1' )) {
            $pack = 'H40';
            $func = 'sha1';
        }
