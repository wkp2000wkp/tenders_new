...
class CDbConnection extends CApplicationComponent {
    ...
    public function init () {
        parent::init();
        if ($this->autoConnect) $this->setActive( 
                true );
    }
    ...
    protected function open () {
        if ($this->_pdo === null) {
            if (empty( 
                    $this->connectionString )) throw new CDbException( 
                    Yii::t( 
                            'yii' , 
    ...
    protected function close () {
        Yii::trace( 'Closing DB connection' , 
                'system.db.CDbConnection' );
        $this->_pdo = null;
        $this->_active = false;
        $this->_schema = null;
    ...
    protected function initConnection ($pdo) {
        ...
        if ($this->emulatePrepare && constant( 
                'PDO::ATTR_EMULATE_PREPARES' )) $pdo->setAttribute( 
    ...
    public function getCurrentTransaction () {
        if ($this->_transaction !== null) {
            if ($this->_transaction
                ->getActive()) return $this->_transaction;
        }
        return null;
    }
    ...
    public function beginTransaction () {
        if ($this->getActive()) {
            $this->_pdo
                ->beginTransaction();
            return $this->_transaction = new CDbTransaction( 
                    $this );
        }
        else
            throw new CDbException( 
    ...
    public function getSchema () {
        if ($this->_schema !== null)
            ...
            if (!$this->getActive()) throw new CDbException( 
                    Yii::t( 
                            'yii' , 
                            'CDbConnection is inactive and cannot perform any DB operations.' ) );
            ...
            switch (strtolower( 
                ...
                case 'pgsql': // PostgreSQL
                    return $this->_schema = new CPgsqlSchema( 
                            $this );
                case 'mysqli': // MySQL
                case 'mysql':
                    return $this->_schema = new CMysqlSchema( 
                            $this );
                case 'sqlite': // sqlite 3
                case 'sqlite2': // sqlite 2
                    return $this->_schema = new CSqliteSchema( 
                            $this );
                case 'mssql': // Mssql driver on windows hosts
                case 'dblib': // dblib drivers on linux (and maybe others os) hosts
                case 'sqlsrv':
                    return $this->_schema = new CMssqlSchema( 
                            $this );
                case 'oci': // Oracle driver
                    return $this->_schema = new COciSchema( 
                            $this );
                case 'ibm':
                default:
                    throw new CDbException( 
    ...
    public function setColumnCase ($value) {
        $this->setAttribute( PDO::ATTR_CASE , 
                $value );
    }
    ...
    public function getStats () {
        ...
        $timings = $logger->getProfilingResults( 
                null , 
