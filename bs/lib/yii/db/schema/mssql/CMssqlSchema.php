...
class CMssqlSchema extends CDbSchema {
    ...
    public function compareTableNames ($name1, $name2) {
        ...
        return parent::compareTableNames( 
                strtolower( 
                        $name1 ) , 
                strtolower( 
                        $name2 ) );
    ...
    protected function createTable ($name) {
        ...
        $table->primaryKey = $this->findPrimaryKey( 
                $table );
        $table->foreignKeys = $this->findForeignKeys( 
                $table );
    ...
    protected function resolveTableNames ($table, $name) {
        ...
        if (($c = count( $parts )) == 3) {
            ...
            $table->name = $parts[2];
            $table->rawName = $this->quoteTableName( 
                    $table->catalogName ) . '.' . $this->quoteTableName( 
                    $table->schemaName ) . '.' . $this->quoteTableName( 
                    $table->name );
        }
        elseif ($c == 2) {
...
EOD;
        $command = $this->getDbConnection()
            ->createCommand( 
                $sql );
        $command->bindValue( ':table' , 
                $table->name );
        $command->bindValue( ':schema' , 
                $table->schemaName );
...
EOD;
    ...
    protected function findColumns ($table) {
        ...
        if (isset( $table->catalogName )) $where[] = "TABLE_CATALOG='" . $table->catalogName . "'";
        if (isset( $table->schemaName )) $where[] = "TABLE_SCHEMA='" . $table->schemaName . "'";
        $sql = "SELECT *, columnproperty(object_id(table_schema+'.'+table_name), column_name, 'IsIdentity') as IsIdentity " . "FROM INFORMATION_SCHEMA.COLUMNS WHERE " . join( 
                ' AND ' , 
                $where );
        if (($columns = $this->getDbConnection()
            ->createCommand( 
                $sql )
            ->queryAll()) === array()) return false;
        foreach ($columns as $column) {
            $c = $this->createColumn( 
                    $column );
            if (is_array( 
                    $table->primaryKey ))
                $c->isPrimaryKey = in_array( 
                        $c->name , 
                        $table->primaryKey );
            else
                $c->isPrimaryKey = strcasecmp( 
                        $c->name , 
                        $table->primaryKey ) === 0;
            $c->isForeignKey = isset( 
                    $table->foreignKeys[$c->name] );
            $table->columns[$c->name] = $c;
            if ($column['IsIdentity'] == 1 && $table->sequenceName === null) $table->sequenceName = $table->name;
        }
        return true;
    }
    ...
    protected function createColumn ($column) {
        ...
        $c->rawName = $this->quoteColumnName( 
                $c->name );
    ...
    protected function findTableNames ($schema = '') {
        if ($schema === '') $schema = self::DEFAULT_SCHEMA;
        $sql = <<<EOD
