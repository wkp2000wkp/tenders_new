...
class CMysqlSchema extends CDbSchema {
    ...
    public function checkIntegrity ($check = true, $schema = '') {
        $this->getDbConnection()
            ->createCommand( 
                'SET FOREIGN_KEY_CHECKS=' . ($check ? 1 : 0) )
            ->execute();
    }
        ...
        foreach ($columns as $column) {
            ...
            if ($c->isPrimaryKey) {
                ...
                if (strpos( 
                        strtolower( 
                                $column['Extra'] ) , 
                        'auto_increment' ) !== false) $table->sequenceName = '';
            }
        }
        return true;
    }
    ...
    protected function findConstraints ($table) {
        ...
        foreach ($matches as $match) {
            ...
            foreach ($keys as $k => $name) {
                ...
                if (isset( 
                        $table->columns[$name] )) $table->columns[$name]->isForeignKey = true;
            }
        }
    }
    ...
    protected function findTableNames ($schema = '') {
        if ($schema === '') return $this->getDbConnection()
            ->createCommand( 
                'SHOW TABLES' )
            ->queryColumn();
        $names = $this->getDbConnection()
            ->createCommand( 
                'SHOW TABLES FROM ' . $this->quoteTableName( 
                        $schema ) )
            ->queryColumn();
