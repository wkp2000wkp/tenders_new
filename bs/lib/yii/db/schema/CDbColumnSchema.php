...
class CDbColumnSchema extends CComponent {
    ...
    public function init ($dbType, $defaultValue) {
        ...
        if ($defaultValue !== null) $this->extractDefault( 
                $defaultValue );
    }
    ...
    protected function extractType ($dbType) {
        if (stripos( $dbType , 'int' ) !== false && stripos( 
                $dbType , 
                'unsigned int' ) === false)
            $this->type = 'integer';
    ...
    protected function extractLimit ($dbType) {
        if (strpos( $dbType , '(' ) && preg_match( 
            ...
            if (isset( 
                    $values[1] )) $this->scale = (int) $values[1];
        }
    }
    ...
    protected function extractDefault ($defaultValue) {
        $this->defaultValue = $this->typecast( 
                $defaultValue );
    }
    ...
    public function typecast ($value) {
        if (gettype( $value ) === $this->type || $value === null || $value instanceof CDbExpression) return $value;
