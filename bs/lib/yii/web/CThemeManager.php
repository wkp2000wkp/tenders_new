...
class CThemeManager extends CApplicationComponent {
    ...
    public function getThemeNames () {
        ...
        if ($themes === null) {
            ...
            while (($file = @readdir( 
                    $folder )) !== false) {
                if ($file !== '.' && $file !== '..' && $file !== '.svn' && is_dir( 
                        $basePath . DIRECTORY_SEPARATOR . $file )) $themes[] = $file;
            }
            closedir( $folder );
            sort( $themes );
        }
    ...
    public function getBasePath () {
        if ($this->_basePath === null) $this->setBasePath( 
                dirname( 
                        Yii::app()->getRequest()
                            ->getScriptFile() ) . DIRECTORY_SEPARATOR . self::DEFAULT_BASEPATH );
    ...
    public function setBasePath ($value) {
        ...
        if ($this->_basePath === false || !is_dir( 
    ...
    public function getBaseUrl () {
        if ($this->_baseUrl === null) $this->_baseUrl = Yii::app()->getBaseUrl() . '/' . self::DEFAULT_BASEPATH;
