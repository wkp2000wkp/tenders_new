...
class CUrlManager extends CApplicationComponent {
    ...
    protected function processRules () {
        ...
        foreach ($this->rules as $pattern => $route)
            $this->_rules[] = $this->createUrlRule( 
                    $route , 
                    $pattern );
        if (isset( $cache )) $cache->set( 
                self::CACHE_KEY , 
                array(
    ...
    public function createUrl ($route, $params = array(), $ampersand = '&') {
        ...
        foreach ($this->_rules as $rule) {
            if (($url = $rule->createUrl( 
                    $this , 
                    $route , 
                    $params , 
                    $ampersand )) !== false) return $rule->hasHostInfo ? $url . $anchor : $this->getBaseUrl() . '/' . $url . $anchor;
        }
    ...
    protected function createUrlDefault ($route, $params, $ampersand) {
        if ($this->getUrlFormat() === self::PATH_FORMAT) {
            ...
            if ($route !== '') {
                ...
                if (($query = $this->createPathInfo( 
                        $params , 
                        '=' , 
                        $ampersand )) !== '') $url .= $ampersand . $query;
            }
            else if (($query = $this->createPathInfo( 
                    $params , 
                    '=' , 
                    $ampersand )) !== '') $url .= '?' . $query;
            return $url;
        ...
        if ($this->getUrlFormat() === self::PATH_FORMAT) {
            ...
            foreach ($this->_rules as $rule) {
                if (($r = $rule->parseUrl( 
                        ...
                        $request , 
                        $pathInfo , 
                        $rawPathInfo )) !== false) return isset( 
    ...
    public function getBaseUrl () {
        if ($this->_baseUrl !== null)
            ...
            if ($this->showScriptName)
                $this->_baseUrl = Yii::app()->getRequest()
                    ->getScriptUrl();
            else
                $this->_baseUrl = Yii::app()->getRequest()
                    ->getBaseUrl();
...
class CUrlRule extends CComponent {
    ...
    public function __construct ($route, $pattern) {
        if (is_array( $route )) {
            if (isset( 
                    $route['urlSuffix'] )) $this->urlSuffix = $route['urlSuffix'];
            if (isset( 
                    $route['caseSensitive'] )) $this->caseSensitive = $route['caseSensitive'];
            if (isset( 
                    $route['defaultParams'] )) $this->defaultParams = $route['defaultParams'];
            if (isset( 
                    $route['matchValue'] )) $this->matchValue = $route['matchValue'];
            $route = $this->route = $route[0];
        }
        else
            $this->route = $route;
        $tr2['/'] = $tr['/'] = '\\/';
        if (strpos( $route , '<' ) !== false && preg_match_all( 
                '/<(\w+)>/' , 
                $route , 
                $matches2 )) {
        ...
        if ($this->append)
            ...
            $this->pattern .= '$/u';
        if ($this->references !== array()) $this->routePattern = '/^' . strtr( 
                $this->route , 
                $tr2 ) . '$/u';
        if (YII_DEBUG && @preg_match( 
                $this->pattern , 
                'test' ) === false) throw new CException( 
                Yii::t( 
                        'yii' , 
    ...
    public function createUrl ($manager, $route, $params, $ampersand) {
        ...
        $tr = array();
        if ($route !== $this->route) {
            if ($this->routePattern !== null && preg_match( 
                    $this->routePattern . $case , 
                    $route , 
                    $matches )) {
        ...
        if ($this->hasHostInfo) {
            $hostInfo = Yii::app()->getRequest()
                ->getHostInfo();
            if (strpos( $url , 
                    $hostInfo ) === 0) $url = substr( 
                    $url , 
                    strlen( 
                            $hostInfo ) );
        }
    ...
    public function parseUrl ($manager, $request, $pathInfo, $rawPathInfo) {
        if ($manager->caseSensitive && $this->caseSensitive === null || $this->caseSensitive)
            ...
            $case = 'i';
        if ($this->urlSuffix !== null) $pathInfo = $manager->removeUrlSuffix( 
                $rawPathInfo , 
                $this->urlSuffix );
        ...
        if ($this->hasHostInfo) $pathInfo = $request->getHostInfo() . rtrim( 
                '/' . $pathInfo , 
                '/' );
        $pathInfo .= '/';
        if (preg_match( $this->pattern . $case , 
                $pathInfo , 
                $matches )) {
            foreach ($this->defaultParams as $name => $value) {
                if (!isset( 
                        $_GET[$name] )) $_REQUEST[$name] = $_GET[$name] = $value;
            }
            $tr = array();
            foreach ($matches as $key => $value) {
                if (isset( 
                    ...
                    $tr[$this->references[$key]] = $value;
                        ...
                        $this->params[$key] )) $_REQUEST[$key] = $_GET[$key] = $value;
            }
            if ($pathInfo !== $matches[0]) // there're additional GET params
$manager->parsePathInfo( 
                    ltrim( 
                            substr( 
                                    $pathInfo , 
                                    strlen( 
