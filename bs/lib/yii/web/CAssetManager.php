 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CAssetManager.php 2400 2010-09-01 13:12:34Z qiang.xue $
 * @package system.web
 * @since 1.0
 */
class CAssetManager extends CApplicationComponent {
    ...
    public function getBasePath () {
        if ($this->_basePath === null) {
            ...
            $this->setBasePath( 
                    dirname( 
                            $request->getScriptFile() ) . DIRECTORY_SEPARATOR . self::DEFAULT_BASEPATH );
        }
    ...
    public function setBasePath ($value) {
        if (($basePath = realpath( $value )) !== false && is_dir( 
                $basePath ) && is_writable( 
                $basePath ))
    ...
    public function getBaseUrl () {
        if ($this->_baseUrl === null) {
            $request = Yii::app()->getRequest();
            $this->setBaseUrl( 
                    $request->getBaseUrl() . '/' . self::DEFAULT_BASEPATH );
        }
    ...
    public function publish ($path, $hashByName = false, $level = -1, $forceCopy = false) {
        if (isset( $this->_published[$path] ))
            ...
            if (is_file( $src )) {
                ...
                $dstDir = $this->getBasePath() . DIRECTORY_SEPARATOR . $dir;
                if (!is_dir( 
                        $dstDir ) || $forceCopy) CFileHelper::copyDirectory( 
                        $src , 
                        $dstDir , 
                        array(
    ...
    public function getPublishedUrl ($path, $hashByName = false) {
        if (isset( $this->_published[$path] )) return $this->_published[$path];
        if (($path = realpath( $path )) !== false) {
            if (is_file( $path ))
                return $this->getBaseUrl() . '/' . $this->hash( 
                        $hashByName ? basename( 
                                $path ) : dirname( 
                                $path ) ) . '/' . basename( 
                        $path );
