...
class CPradoViewRenderer extends CViewRenderer {
    ...
    protected function generateViewFile ($sourceFile, $viewFile) {
        ...
        for ($i = 0; $i < $n; ++$i) {
            ...
            $matchEnd = $matchStart + strlen( 
                    $str ) - 1;
            if ($matchStart > $textStart) $this->_output .= substr( 
                    $this->_input , 
                    $textStart , 
                    $matchStart - $textStart );
            $textStart = $matchEnd + 1;
            if (strpos( $str , 
                    '<com:' ) === 0) // opening component tag
{
                $type = $match[3][0];
                if ($str[strlen( 
                        $str ) - 2] !== '/') // open tag
                    $this->_output .= $this->processBeginWidget( 
                            $type , 
                            $match[4][0] , 
                            $match[2][1] );
                else
                    $this->_output .= $this->processWidget( 
                            $type , 
                            $match[4][0] , 
                            $match[2][1] );
            }
                    ...
                    $str , 
                    '</com:' ) === 0) // closing component tag
                $this->_output .= $this->processEndWidget( 
                        $match[3][0] , 
                        $match[2][1] );
            else if (strpos( 
                    $str , 
                    '<cache:' ) === 0) // opening cache tag
{
                $id = $match[3][0];
                if ($str[strlen( 
                        $str ) - 2] !== '/') // open tag
                    $this->_output .= $this->processBeginCache( 
                            $id , 
                            $match[4][0] , 
                            $match[2][1] );
                else
                    $this->_output .= $this->processCache( 
                            $id , 
                            $match[4][0] , 
                            $match[2][1] );
            }
                    ...
                    $str , 
                    '</cache:' ) === 0) // closing cache tag
                $this->_output .= $this->processEndCache( 
                        $match[3][0] , 
                        $match[2][1] );
            else if (strpos( 
                    $str , 
                    '<clip:' ) === 0) // opening clip tag
{
                $id = $match[3][0];
                if ($str[strlen( 
                        $str ) - 2] !== '/') // open tag
                    $this->_output .= $this->processBeginClip( 
                            $id , 
                            $match[4][0] , 
                            $match[2][1] );
                else
                    $this->_output .= $this->processClip( 
                            $id , 
                            $match[4][0] , 
                            $match[2][1] );
            }
                    ...
                    $str , 
                    '</clip:' ) === 0) // closing clip tag
                $this->_output .= $this->processEndClip( 
                        $match[3][0] , 
                        $match[2][1] );
            else if (strpos( 
                    $str , 
                    '<%=' ) === 0) // expression
                $this->_output .= $this->processExpression( 
                        $match[1][0] , 
                        $match[1][1] );
            else if (strpos( 
                    $str , 
...
$this->_output .= $this->processStatement( 
        ...
        if ($textStart < strlen( $this->_input )) $this->_output .= substr( 
                $this->_input , 
                $textStart );
    ...
    private function processBeginWidget ($type, $attributes, $offset) {
        ...
        if (empty( $attrs ))
            return $this->generatePhpCode( 
                    "\$this->beginWidget('$type');" , 
                    $offset );
        else
            return $this->generatePhpCode( 
                    "\$this->beginWidget('$type', array($attrs));" , 
                    $offset );
    }
    ...
    private function processCache ($id, $attributes, $offset) {
        return $this->processBeginCache( $id , 
                $attributes , 
                $offset ) . $this->processEndCache( 
                $id , 
                $offset );
    }
    ...
    private function processBeginCache ($id, $attributes, $offset) {
        ...
        if (empty( $attrs ))
            ...
            return $this->generatePhpCode( 
                    "if(\$this->beginCache('$id', array($attrs))):" , 
                    $offset );
    }
    ...
    private function processClip ($id, $attributes, $offset) {
        return $this->processBeginClip( $id , 
                $attributes , 
                $offset ) . $this->processEndClip( 
                $id , 
                $offset );
    }
