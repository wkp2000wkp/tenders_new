...
require_once (Yii::getPathOfAlias( 
        'system.vendors.TextHighlighter.Text.Highlighter' ) . '.php');
require_once (Yii::getPathOfAlias( 
        'system.vendors.TextHighlighter.Text.Highlighter.Renderer.Html' ) . '.php');
/**
 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CTextHighlighter.php 1678 2010-01-07 21:02:00Z qiang.xue $
...
class CTextHighlighter extends COutputProcessor {
    ...
    public function highlight ($content) {
        ...
        if ($this->showLineNumbers) $options['numbers'] = ($this->lineNumberStyle === 'list') ? HL_NUMBERS_LI : HL_NUMBERS_TABLE;
        $highlighter = empty( $this->language ) ? false : Text_Highlighter::factory( 
                $this->language );
        if ($highlighter === false)
            $o = '<pre>' . CHtml::encode( 
                    $content ) . '</pre>';
        else {
            $highlighter->setRenderer( 
                    new Text_Highlighter_Renderer_Html( 
                            $options ) );
            $o = preg_replace( 
                    '/<span\s+[^>]*>(\s*)<\/span>/' , 
                    '\1' , 
    ...
    public function registerClientScript () {
        if ($this->cssFile !== false) self::registerCssFile( 
                $this->cssFile );
    }
    ...
    public static function registerCssFile ($url = null) {
        if ($url === null) $url = CHtml::asset( 
                Yii::getPathOfAlias( 
                        'system.vendors.TextHighlighter.highlight' ) . '.css' );
