...
class CClientScript extends CApplicationComponent {
    ...
    public function render (&$output) {
        ...
        if (!empty( $this->scriptMap )) $this->remapScripts();
    ...
    protected function remapScripts () {
        ...
        foreach ($this->cssFiles as $url => $media) {
            ...
            if (isset( 
                    $this->scriptMap[$name] )) {
                if ($this->scriptMap[$name] !== false) $cssFiles[$this->scriptMap[$name]] = $media;
            }
            else if (isset( 
                    $this->scriptMap['*.css'] )) {
                if ($this->scriptMap['*.css'] !== false) $cssFiles[$this->scriptMap['*.css']] = $media;
            }
            else
                $cssFiles[$url] = $media;
        }
        ...
        foreach ($this->scriptFiles as $position => $scripts) {
            ...
            foreach ($scripts as $key => $script) {
                ...
                if (isset( 
                        $this->scriptMap[$name] )) {
                    if ($this->scriptMap[$name] !== false) $jsFiles[$position][$this->scriptMap[$name]] = $this->scriptMap[$name];
                }
                else if (isset( 
                        $this->scriptMap['*.js'] )) {
                    if ($this->scriptMap['*.js'] !== false) $jsFiles[$position][$this->scriptMap['*.js']] = $this->scriptMap['*.js'];
                }
                else
        ...
        if ($this->enableJavaScript) {
            if (isset( 
                ...
                foreach ($this->scriptFiles[self::POS_HEAD] as $scriptFile)
                    $html .= CHtml::scriptFile( 
                            $scriptFile ) . "\n";
            }
            if (isset( 
                    $this->scripts[self::POS_HEAD] )) $html .= CHtml::script( 
                    implode( 
                            "\n" , 
    ...
    public function renderBodyBegin (&$output) {
        ...
        if (isset( 
                $this->scripts[self::POS_BEGIN] )) $html .= CHtml::script( 
                implode( 
                        "\n" , 
    ...
    public function renderBodyEnd (&$output) {
        if (!isset( 
                $this->scriptFiles[self::POS_END] ) && !isset( 
                $this->scripts[self::POS_END] ) && !isset( 
                $this->scripts[self::POS_READY] ) && !isset( 
                $this->scripts[self::POS_LOAD] )) return;
        $fullPage = 0;
        $output = preg_replace( 
                '/(<\\/body\s*>)/is' , 
                '<###end###>$1' , 
        ...
        $scripts = isset( 
                $this->scripts[self::POS_END] ) ? $this->scripts[self::POS_END] : array();
        if (isset( 
        ...
        if (!empty( $scripts )) $html .= CHtml::script( 
                implode( 
                        "\n" , 
                        $scripts ) ) . "\n";
        if ($fullPage)
    ...
    public function getCoreScriptUrl () {
        if ($this->_baseUrl !== null)
            ...
            return $this->_baseUrl = Yii::app()->getAssetManager()
                ->publish( 
    ...
    public function registerScript ($id, $script, $position = self::POS_READY) {
        ...
        if ($position === self::POS_READY) $this->registerCoreScript( 
                'jquery' );
        $params = func_get_args();
        $this->recordCachingAction( 
                'clientScript' , 
    ...
    protected function recordCachingAction ($context, $method, $params) {
        if (($controller = Yii::app()->getController()) !== null) $controller->recordCachingAction( 
                $context , 
                $method , 
                $params );
    }
