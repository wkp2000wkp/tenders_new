 ...
 * @since 1.0
 */
class CMap extends CComponent implements 
    ...
    public function contains ($key) {
        return isset( $this->_d[$key] ) || array_key_exists( 
                $key , 
                $this->_d );
    }
    ...
    public function copyFrom ($data) {
        if (is_array( $data ) || $data instanceof Traversable) {
            ...
            if ($data instanceof CMap) $data = $data->_d;
            foreach ($data as $key => $value)
                $this->add( 
                        $key , 
                        $value );
        }
        else if ($data !== null) throw new CException( 
                Yii::t( 
                        'yii' , 
