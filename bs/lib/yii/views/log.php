...
<table class="yiiLog" width="100%" cellpadding="2"
	...
	<tr>
		<th style="background: black; color: white;" colspan="5">Application
		Log</th>
	</tr>
...
foreach ($data as $index => $log) {
    ...
    if (isset( $colors[$log[1]] )) $color = $colors[$log[1]];
    $message = '<pre>' . CHtml::encode( wordwrap( $log[0] ) ) . '</pre>';
    $time = date( 'H:i:s.' , $log[3] ) . sprintf( '%06d' , 
            (int) (($log[3] - (int) $log[3]) * 1000000) );
    echo <<<EOD
